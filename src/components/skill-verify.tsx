    ...
    <div className="space-y-5">
      ...
      <div>
        <div className="flex items-baseline gap-3 mb-1">
          ...
          <h2 className="text-[17px] text-[#0F1C3F]" style={{ fontFamily: "var(--font-serif)" }}>Skill Verification</h2>
          <span className="text-[11px] font-semibold text-emerald-700 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-full flex items-center gap-1"><ShieldCheck size={10} /> Only verified skills appear on your CV</span>
        </div>
        ...
        {[
          ...
          return (
            <button key={s.label} onClick={() => setFilter(s.label === "Verified" ? "verified" : s.label === "In Progress" ? "in_progress" : "not_started")}
              className={`flex items-center gap-3 px-4 py-3 rounded-xl border-2 transition-all hover:shadow-sm ${s.color}`}>
      ...
      <div>
        <div className="flex items-center justify-between mb-1.5">
          <span className="text-[12.5px] font-medium text-[#0F1C3F]">Overall Verification Progress</span>
          <span className="text-[12.5px] font-bold text-[#1B3A6B]">{verifiedCount}/{skills.length} skills verified</span>
        </div>
      ...
      <div className="grid gap-4" style={{ gridTemplateColumns: "220px 1fr" }}>
          ...
          {filteredSkills.map(sk => {
            ...
            return (
              <button key={sk.id} onClick={() => { setSelectedId(sk.id); setQuizStep(0); setGhResult(false); setMentorSent(false); setAdminSent(false); setCodeSubmitted(false); }}
                className={`w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl text-left transition-all border-2 ${active ? "border-[#1B3A6B] bg-[#EBF1FA]" : "border-transparent hover:bg-[#F4F7FC]"}`}>
        ...
        <div className="bg-white border-2 border-[--border] rounded-2xl overflow-hidden">
          ...
          {/* Skill header */}
          <div className={`px-5 py-4 border-b border-[--border] flex items-center justify-between ${selected.status === "verified" ? "bg-emerald-50" : selected.status === "in_progress" ? "bg-amber-50" : "bg-[#F8FAFB]"}`}>
            <div>
              <div className="flex items-center gap-2.5">
                <p className="text-[16px] font-bold text-[#0F1C3F]">{selected.name}</p>
          ...
          <div className="p-5">
                  ...
                  {VERIFY_METHODS.map(m => {
                    ...
                    return (
                      <button key={m.id} onClick={() => setMethod(m.id as VerifyMethod)}
                        className={`flex flex-col items-start gap-1.5 px-3 py-2.5 rounded-xl border-2 text-left transition-all ${active ? "border-[#1B3A6B] bg-[#EBF1FA]" : "border-[--border] bg-white hover:border-slate-300"}`}>
                        ...
                        <p className="text-[10.5px] text-[#9AA5BE] leading-tight">{m.desc}</p>
                      </button>
                    );
                  })}
                </div>
              ...
              <div>
                {quizStep === 0 && (
                  <div className="p-4 bg-[#EBF1FA] rounded-xl text-center">
                    ...
                    <p className="text-[12px] text-[#5A6A8A] mb-4">15 multiple-choice questions · ~10 minutes · Score ≥70% to pass</p>
            ...
            {selected.status !== "verified" && selected.method === "coding" && (
              <div className="space-y-4">
                <div className="p-4 bg-[#F8FAFB] rounded-xl border border-[--border]">
                  <div className="flex items-center justify-between mb-2">
                    <p className="text-[13.5px] font-semibold text-[#0F1C3F]">Challenge: Array Manipulation with {selected.name}</p>
                ...
                {!codeSubmitted ? (
                  <button onClick={() => { setCodeSubmitted(true); setTimeout(() => markVerified(82), 1200); }}
                    className="flex items-center gap-2 px-5 py-2.5 bg-[#1B3A6B] text-white text-[13px] rounded-xl hover:bg-[#122748] transition-colors font-semibold">
                    ...
                    <div className="p-4 bg-[#F8FAFB] border border-[--border] rounded-xl">
                      ...
                      <div className="grid grid-cols-3 gap-3 mb-3">
                        {[
                          ...
                          <div key={s.label} className="bg-white border border-[--border] rounded-lg px-3 py-2 text-center">
      ...
      {/* CV visibility note */}
      <div className="flex items-start gap-3 p-4 bg-[#EBF1FA] border border-[#1B3A6B]/15 rounded-xl">
