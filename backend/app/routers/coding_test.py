...
from ..services.certificate import auto_issue_if_eligible
from ..services.code_execution import ALGORITHMIC_LANGUAGES, grade_submission
from ..services.enrollment import get_active_enrollment
from ..schemas.coding_test import CodingRunIn, CodingTestUpsertIn
...
async def submit_coding_test(coding_test_id: uuid.UUID, payload: CodingRunIn,
    ...
    public_results = []
    for case, result in zip(test_cases, results):
        safe_result = {"passed": result["passed"]}
        ...
        public_results.append(
            {"test_case_id": case.id, "title": case.title, "is_hidden": case.is_hidden, **safe_result}
        )
