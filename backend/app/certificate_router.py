...
from .certificate_service import is_eligible, issue_certificate as issue_certificate_record
from .config import get_settings
from .database import get_db
from .dependencies import get_current_admin, get_current_student
from .models import Certificate, CertificateEvent, CourseEnrollment, User
...
def issue_certificate(enrollment_id: uuid.UUID, response: Response,
    ...
    item = issue_certificate_record(enrollment, user.id, db); db.commit(); db.refresh(item)
    return _out(item)
...
def reissue_certificate(certificate_id: uuid.UUID, admin: User = Depends(get_current_admin), db: Session = Depends(get_db)):
    ...
    replacement = issue_certificate_record(enrollment, admin.id, db, parent=item)
