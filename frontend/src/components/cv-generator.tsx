...
export function SSCVGenerator() {
  ...
  const [cv, setCV] = useState<CVData>({
    ...
    summary:
      "Final-year B.E. Computer Engineering student with hands-on experience in full-stack web development using React and Node.js. Demonstrated ability to build and deploy production-grade applications. Seeking a Full Stack Developer role to contribute technical skills within a high-growth engineering team.",
    education: [
      {
  ...
  const toggleSection = (id: string) =>
    setSections((s) =>
      s.map((x) => (x.id === id ? { ...x, visible: !x.visible } : x)),
    );
  ...
  const atsChecks = [
    {
      ...
      pass: !!(cv.email && cv.phone && cv.linkedin),
    },
  ...
  const atsScore = Math.round(
    (atsChecks.filter((c) => c.pass).length / atsChecks.length) * 100,
  );
  ...
  const exportDOCX = async () => {
    ...
    const bullet = (text: string) =>
      new Para({
        children: [
          new TR({ text, size: 20, font: "Calibri", color: "374151" }),
        ],
        bullet: { level: 0 },
        spacing: { after: 60 },
      });
    ...
    const doc = new Doc({
      styles: {
        default: {
          document: { run: { font: "Calibri", size: 20, color: "374151" } },
        },
      },
      sections: [
        {
                  ...
                  ...cv.education
                    .map((e) => [
                      ...
                      new Para({
                        children: [
                          new TR({
                            text: `${e.college}  |  ${e.year}  |  CGPA: ${e.cgpa}`,
                            size: 20,
                            font: "Calibri",
                            color: "5A6A8A",
                          }),
            ...
            ...(visibleSections.includes("skills")
              ? [
                  ...
                  new Para({
                    children: [
                      new TR({
                        text: `Frameworks & Libraries: ${cv.skills.technical.join(", ")}`,
                        size: 20,
                        font: "Calibri",
                        color: "374151",
                      }),
                  ...
                  new Para({
                    children: [
                      new TR({
                        text: `Tools & Platforms: ${cv.skills.tools.join(", ")}`,
                        size: 20,
                        font: "Calibri",
                        color: "374151",
                      }),
                  ...
                  ...cv.experience
                    .map((e) => [
                      ...
                      new Para({
                        children: [
                          new TR({
                            text: `${e.company}  |  ${e.duration}  |  ${e.location}`,
                            size: 20,
                            font: "Calibri",
                            color: "5A6A8A",
                          }),
            ...
            ...(visibleSections.includes("readiness")
              ? [
                  ...
                  new Para({
                    children: [
                      new TR({
                        text: `Overall Readiness: ${cv.readiness.overall}%  |  ${cv.readiness.role}: ${cv.readiness.roleScore}%`,
                        size: 20,
                        font: "Calibri",
                        color: "374151",
                      }),
            ...
            ...(visibleSections.includes("profile")
              ? [
                  ...
                  new Para({
                    children: [
                      new TR({
                        text: `EduConnect Profile: ${cv.publicUrl}`,
                        size: 20,
                        font: "Calibri",
                        color: "1B3A6B",
                      }),
  ...
  const exportPDF = () => {
    ...
    const certHtml = cv.certifications
      .map(
        (c) =>
          `<li style="font-size:10pt;margin-bottom:3pt">${c.name} &mdash; ${c.issuer} (${c.year})</li>`,
      )
      .join("");
    const assHtml = cv.assessments
      .map(
        (a) =>
          `<li style="font-size:10pt;margin-bottom:3pt">${a.name}: <strong>${a.score}/${a.max}</strong></li>`,
      )
      .join("");
    ...
    const html = `<!DOCTYPE html><html><head><meta charset="UTF-8">
      ...
      ${
        ...
        <div style="color:#555;font-size:10pt">${e.college} &nbsp;|&nbsp; ${e.year} &nbsp;|&nbsp; CGPA: ${e.cgpa}</div></div>`,
                )
                .join(""),
            )
          : ""
  ...
  const renderEditor = () => {
    ...
    if (activeSec === "summary")
      return (
        <Field
          label="Professional Summary"
          hint="2–3 sentences, keyword-rich, ATS-optimised"
        >
          <textarea
            rows={5}
            value={cv.summary}
          ...
          <p className="text-[11px] text-[#9AA5BE] mt-1">
            {cv.summary.length} characters — aim for 300–600
          </p>
        </Field>
      );

    if (activeSec === "education")
      return (
        <div className="grid grid-cols-2 gap-3">
          {inputRow(
            "Degree / Course",
          ...
          {inputRow(
            "College / University",
            "education.0.college",
            cv.education[0].college,
          )}
          {inputRow(
            "Affiliated Board",
            "education.0.board",
            cv.education[0].board,
          )}
    ...
    if (activeSec === "skills")
      return (
        <div className="space-y-3">
            ...
            >
              <TagInput
                ...
                onAdd={(t) =>
                  setCV((p) => ({
                    ...p,
                    skills: { ...p.skills, [k]: [...p.skills[k], t] },
                  }))
                }
                onRemove={(t) =>
                  setCV((p) => ({
          ...
          <InfoBox title="ATS Tip" variant="blue">
            List skills as comma-separated keywords. Avoid visual chip displays
            — they are stripped by ATS parsers.
          </InfoBox>
        </div>
      );

    if (activeSec === "experience")
      return (
        <div className="space-y-3">
            ...
            <textarea
              ...
              onChange={(e) =>
                setCV((p) => ({
                  ...p,
                  experience: [
                    { ...p.experience[0], bullets: e.target.value.split("\n") },
          ...
          <InfoBox title="ATS Tip" variant="blue">
            Start each bullet with an action verb (Developed, Implemented,
            Designed, Led). Include measurable outcomes.
          </InfoBox>
        </div>
      );

    if (activeSec === "projects")
      return (
        <div className="space-y-4">
          {cv.projects.map((p, i) => (
            <div
              key={i}
              className="p-3 bg-[#F4F6FB] rounded-xl border border-[--border] space-y-3"
            >
              <p className="text-[12px] font-semibold text-[#1B3A6B]">
                Project {i + 1}
              </p>
              <div className="grid grid-cols-2 gap-3">
                <Field label="Title">
    ...
    return (
      <div className="flex flex-col items-center justify-center h-24 text-[#9AA5BE]">
        <p className="text-[12.5px]">
          This section is auto-populated from your profile data.
        </p>
      </div>
    );
  };
  ...
  const CVPreview = () => {
    const sec = (title: string, children: React.ReactNode) => (
      <div className="mb-4">
        <div className="text-[9.5pt] font-bold tracking-widest text-[#1B3A6B] uppercase border-b-2 border-[#1B3A6B] pb-0.5 mb-2">
          {title}
        </div>
        {children}
      </div>
    ...
    const bul = (text: string) => (
      <div key={text} className="flex gap-2 text-[9pt] text-[#374151] mb-0.5">
    ...
    return (
      <div
        className="bg-white shadow-lg border border-slate-200 rounded-sm"
        style={{
      ...
      >
        {/* Name */}
        <div className="text-[22pt] font-bold text-[#1B3A6B] leading-none mb-1">
          {cv.name}
        </div>
        {/* Contact */}
        <div className="text-[8.5pt] text-[#5A6A8A] border-b border-slate-300 pb-2 mb-3 flex flex-wrap gap-x-3 gap-y-0.5">
          {[cv.email, cv.phone, cv.linkedin, cv.github, cv.location]
            .filter(Boolean)
            .map((c, i, a) => (
              <span key={c}>
                {c}
        ...
        {visibleSections.includes("summary") &&
          sec(
            ...
            <p className="text-[9.5pt] text-[#374151] leading-relaxed">
              {cv.summary}
            </p>,
          )}
          ...
          sec(
            ...
            cv.education.map((e, i) => (
              <div key={i} className="flex justify-between items-start mb-1">
                <div>
                  <div className="text-[10pt] font-bold text-[#0F1C3F]">
                    {e.degree}
                  </div>
                  <div className="text-[9pt] text-[#5A6A8A]">
                    {e.college} &nbsp;|&nbsp; Affiliated: {e.board}
                  </div>
                </div>
                <div className="text-right shrink-0 ml-4">
                  <div className="text-[9pt] font-semibold text-[#0F1C3F]">
                    {e.year}
                  </div>
                  <div className="text-[9pt] text-[#5A6A8A]">
                    CGPA: {e.cgpa}
        ...
        {visibleSections.includes("skills") &&
          sec(
            ...
            <div className="space-y-0.5 text-[9.5pt] text-[#374151]">
              <div>
                <span className="font-semibold text-[#0F1C3F]">Languages:</span>{" "}
            ...
            cv.experience.map((e, i) => (
              <div key={i} className="mb-3">
                <div className="flex justify-between items-baseline">
                  <div className="text-[10.5pt] font-bold text-[#0F1C3F]">
                    {e.title}
                  </div>
                  <div className="text-[9pt] text-[#5A6A8A] shrink-0 ml-3">
                    {e.duration}
                  </div>
                </div>
                <div className="text-[9pt] text-[#5A6A8A] mb-1">
          ...
          sec(
            ...
            cv.projects.map((p, i) => (
              <div key={i} className="mb-3">
                ...
                <div className="text-[9pt] text-[#5A6A8A] mb-1">
                  Tech Stack: {p.tech} &nbsp;|&nbsp;{" "}
                  <span className="text-[#1B3A6B]">{p.link}</span>
                </div>
                {p.bullets.map(bul)}
              </div>
            )),
            ...
            cv.certifications.map((c) => (
              ...
              >
                <div className="flex gap-2">
                  <span>•</span>
                  <span className="text-[#0F1C3F]">{c.name}</span>
                  <span className="text-[#5A6A8A]">— {c.issuer}</span>
                </div>
            ...
            <div className="flex flex-wrap gap-x-6 gap-y-1">
              {cv.assessments.map((a) => (
                <div key={a.name} className="text-[9.5pt]">
                  <span className="text-[#0F1C3F] font-semibold">
                    {a.name}:
                  </span>{" "}
        ...
        {visibleSections.includes("readiness") &&
          sec(
            ...
            <div className="text-[9.5pt] text-[#374151]">
              Overall Readiness:{" "}
              <span className="font-semibold">{cv.readiness.overall}%</span>{" "}
              &nbsp;|&nbsp;
              {cv.readiness.role}:{" "}
              <span className="font-semibold">{cv.readiness.roleScore}%</span>
            </div>,
          )}
          ...
          sec(
            ...
            <div className="text-[9.5pt]">
              <div>
                ...
                <span className="text-[#1B3A6B]">{cv.publicUrl}</span>
              </div>
              <div>
                <span className="text-[#0F1C3F] font-semibold">Portfolio:</span>{" "}
                <span className="text-[#1B3A6B]">{cv.portfolio}</span>
              </div>
            </div>,
          )}
      </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between">
        <div>
          <div className="flex items-baseline gap-3 mb-1">
            ...
            <h2
              className="text-[17px] text-[#0F1C3F]"
            ...
            <span className="flex items-center gap-1 text-[11px] font-semibold text-emerald-700 bg-emerald-50 border border-emerald-200 px-2 py-0.5 rounded-full">
              <ShieldCheck size={10} /> ATS-Optimised
            </span>
          </div>
          <p className="text-[12.5px] text-[#5A6A8A]">
            Auto-generated from your verified profile. Toggle sections, edit
            inline, then export.
          </p>
        </div>
        {/* ATS Score */}
        <div className="flex items-center gap-3 bg-white border border-[--border] rounded-xl px-4 py-3 shadow-sm shrink-0">
          <div>
            <p className="text-[10px] font-semibold text-[#9AA5BE] uppercase tracking-wide">
              ATS Score
            </p>
            <p
          ...
          <div className="space-y-1 min-w-[140px]">
            {atsChecks.slice(0, 5).map((c) => (
              <div key={c.label} className="flex items-center gap-1.5">
                ...
                <span className="text-[10px] text-[#5A6A8A] truncate">
                  {c.label}
                </span>
              </div>
            ))}
      ...
      <div className="flex items-center gap-3 p-4 bg-white border border-[--border] rounded-xl shadow-sm">
        <span className="text-[12.5px] font-semibold text-[#0F1C3F] mr-1">
          Export:
        </span>
        <button
          onClick={exportPDF}
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          <div className="flex-1 flex items-center gap-2 px-3 py-2 bg-[#EFF2FA] rounded-[10px] min-w-0">
            ...
            <span
              className="text-[12px] text-[#1B3A6B] truncate"
              style={{ fontFamily: "var(--font-mono)" }}
            >
              {cv.publicUrl}
            </span>
      ...
      <div className="grid gap-5" style={{ gridTemplateColumns: "280px 1fr" }}>
        ...
        <div className="space-y-2">
          <p className="text-[11.5px] font-semibold text-[#9AA5BE] uppercase tracking-wide px-1">
            Sections ({visibleSections.length} visible)
          </p>
          {sections.map((sec) => {
            const Icon = sec.icon;
            ...
            return (
              ...
              >
                <div
                  className={`flex items-center gap-2.5 px-3 py-2.5 cursor-pointer ${isActive ? "bg-[#EBF1FA]" : "bg-white hover:bg-[#F8FAFB]"}`}
                  onClick={() =>
                    setActiveSec(activeSec === sec.id ? null : sec.id)
                  }
                >
                  <div
                    className={`w-7 h-7 rounded-lg flex items-center justify-center shrink-0 ${sec.visible ? "bg-[#1B3A6B]" : "bg-[#E8ECF5]"}`}
                  >
                    <Icon
                      size={12}
                      className={sec.visible ? "text-white" : "text-[#9AA5BE]"}
                    />
                  </div>
                  <span
                    className={`flex-1 text-[12.5px] font-medium truncate ${isActive ? "text-[#1B3A6B]" : sec.visible ? "text-[#0F1C3F]" : "text-[#9AA5BE]"}`}
                  >
                    {sec.label}
                  </span>
                  <button
                  ...
                  >
                    <div
                      className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-all ${sec.visible ? "right-0.5" : "left-0.5"}`}
                    />
                  </button>
                </div>
                {isActive && (
          ...
          <div className="mt-3 p-3 bg-white border border-[--border] rounded-xl">
            <p className="text-[11.5px] font-semibold text-[#0F1C3F] mb-2">
              ATS Checklist
            </p>
            {atsChecks.map((c) => (
              <div key={c.label} className="flex items-center gap-2 py-0.5">
                ...
                <span
                  className={`text-[11px] ${c.pass ? "text-[#5A6A8A]" : "text-red-500"}`}
                >
                  {c.label}
                </span>
              </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-3">
            <p className="text-[12px] font-semibold text-[#5A6A8A] uppercase tracking-wide">
              Live ATS Preview
            </p>
