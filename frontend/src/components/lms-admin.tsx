...
function AssignmentBuilder({lessonId,onClose}:{lessonId:string;onClose:()=>void}){
  ...
  const save=async()=>{setBusy(true);setError("");try{const a=await saveAdminAssignment(lessonId,form);setId(a.id);setStatus(a.status);}catch(e){setError(errText(e));}finally{setBusy(false);}};
  const publish=async()=>{if(!id)return;setBusy(true);setError("");try{const a=await publishAdminAssignment(id);setStatus(a.status);}catch(e){setError(errText(e));}finally{setBusy(false);}};
    ...
    <div className="min-h-screen bg-[#F2F5FC] text-[#0F1C3F]" style={{ fontFamily: "var(--font-sans)" }}>
      ...
      <header className="sticky top-0 z-20 bg-[#0F1F3B] text-white border-b border-white/5">
        <div className="px-6 py-4 flex items-center justify-between">
          ...
          <div className="flex gap-2"><button onClick={()=>setCertificatesOpen(true)} className="px-3 py-2 rounded-xl bg-white/10 text-[12px] font-semibold flex items-center gap-2"><ShieldCheck size={14}/>Certificates</button><button onClick={()=>setSubmissionsOpen(true)} className="px-3 py-2 rounded-xl bg-white/10 text-[12px] font-semibold flex items-center gap-2"><Eye size={14}/>Review Submissions</button><button onClick={() => { setSelectedId(null); setDetail(null); setCourseForm(blankCourse); setEditorOpen(true); }} className="px-3 py-2 rounded-xl bg-white/10 text-[12px] font-semibold flex items-center gap-2"><Plus size={14} />New Course</button></div>
        </div>
      </header>
