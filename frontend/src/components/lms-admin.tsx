...
const ADMIN_PAGE_PATH:Record<AdminPage,string>={dashboard:"/admin/dashboard",courses:"/admin/courses","add-course":"/admin/courses/new"};
function adminPageFromPath(pathname:string):AdminPage{return pathname==="/admin/courses/new"?"add-course":pathname.startsWith("/admin/courses")?"courses":"dashboard";}
...
export function LMSAdminSection({ onBack, onLogout }: { onBack?: () => void | Promise<void>; onLogout?: () => void | Promise<void> }) {
  const [adminPage,setAdminPage]=useState<AdminPage>(()=>adminPageFromPath(window.location.pathname));
  ...
  const [submissionsOpen,setSubmissionsOpen]=useState(()=>window.location.pathname==="/admin/submissions");
  const [certificatesOpen,setCertificatesOpen]=useState(()=>window.location.pathname==="/admin/certificates");
  const [institutionOpen,setInstitutionOpen]=useState(()=>window.location.pathname==="/admin/institution");
  const [analytics,setAnalytics]=useState<AdminAnalyticsOverview|null>(null);
  const navigateAdmin=(next:AdminPage,path=ADMIN_PAGE_PATH[next])=>{if(window.location.pathname!==path)window.history.pushState({},"",path);setAdminPage(next);setSubmissionsOpen(false);setCertificatesOpen(false);setInstitutionOpen(false);setSidebarOpen(false);window.scrollTo({top:0,behavior:"smooth"});};
  const openSubmissions=()=>{if(window.location.pathname!=="/admin/submissions")window.history.pushState({},"","/admin/submissions");setSubmissionsOpen(true);setCertificatesOpen(false);setInstitutionOpen(false);setSidebarOpen(false);};
  const openCertificates=()=>{if(window.location.pathname!=="/admin/certificates")window.history.pushState({},"","/admin/certificates");setCertificatesOpen(true);setSubmissionsOpen(false);setInstitutionOpen(false);setSidebarOpen(false);};
  const openInstitution=()=>{if(window.location.pathname!=="/admin/institution")window.history.pushState({},"","/admin/institution");setInstitutionOpen(true);setSubmissionsOpen(false);setCertificatesOpen(false);setSidebarOpen(false);};
  ...
  const loadDetail = async (courseId: string) => {
    const coursePath=`/admin/courses/${courseId}`;if(window.location.pathname!==coursePath)window.history.pushState({},"",coursePath);
  ...
  useEffect(()=>{if(window.location.pathname==="/"||window.location.pathname==="/admin")window.history.replaceState({},"",ADMIN_PAGE_PATH.dashboard);const courseId=window.location.pathname.match(/^\/admin\/courses\/([^/]+)$/)?.[1];if(courseId&&courseId!=="new")void loadDetail(courseId);const restore=()=>{const path=window.location.pathname;setAdminPage(adminPageFromPath(path));setSubmissionsOpen(path==="/admin/submissions");setCertificatesOpen(path==="/admin/certificates");setInstitutionOpen(path==="/admin/institution");const id=path.match(/^\/admin\/courses\/([^/]+)$/)?.[1];if(id&&id!=="new")void loadDetail(id);};window.addEventListener("popstate",restore);return()=>window.removeEventListener("popstate",restore);},[]);
  useEffect(()=>{document.title=`${submissionsOpen?"Submissions":certificatesOpen?"Certificates":institutionOpen?"Institution":adminPage==="add-course"?"Add Course":adminPage==="courses"?"Courses":"Dashboard"} | EduConnect Admin`;},[adminPage,submissionsOpen,certificatesOpen,institutionOpen]);
  ...
  const openNewCourse=()=>{setSelectedId(null);setDetail(null);setCourseForm(blankCourse);setEditorOpen(true);setCourseStep(1);navigateAdmin("add-course");setSidebarOpen(false);setError("");setNotice("");};
  const navigation=[
