...
export function LMSAdminSection({ onBack, onLogout }: { onBack?: () => void | Promise<void>; onLogout?: () => void | Promise<void> }) {
  ...
  const navigateAdmin=(next:AdminPage,path=ADMIN_PAGE_PATH[next])=>{if(window.location.pathname!==path)window.history.pushState({},"",path);setAdminPage(next);setSubmissionsOpen(false);setCertificatesOpen(false);setSidebarOpen(false);window.scrollTo({top:0,behavior:"smooth"});};
  const openSubmissions=()=>{if(window.location.pathname!=="/admin/submissions")window.history.pushState({},"","/admin/submissions");setSubmissionsOpen(true);setCertificatesOpen(false);setSidebarOpen(false);};
  const openCertificates=()=>{if(window.location.pathname!=="/admin/certificates")window.history.pushState({},"","/admin/certificates");setCertificatesOpen(true);setSubmissionsOpen(false);setSidebarOpen(false);};
  ...
  useEffect(()=>{if(window.location.pathname==="/"||window.location.pathname==="/admin")window.history.replaceState({},"",ADMIN_PAGE_PATH.dashboard);const courseId=window.location.pathname.match(/^\/admin\/courses\/([^/]+)$/)?.[1];if(courseId&&courseId!=="new")void loadDetail(courseId);const restore=()=>{const path=window.location.pathname;setAdminPage(adminPageFromPath(path));setSubmissionsOpen(path==="/admin/submissions");setCertificatesOpen(path==="/admin/certificates");const id=path.match(/^\/admin\/courses\/([^/]+)$/)?.[1];if(id&&id!=="new")void loadDetail(id);};window.addEventListener("popstate",restore);return()=>window.removeEventListener("popstate",restore);},[]);
  useEffect(()=>{document.title=`${submissionsOpen?"Submissions":certificatesOpen?"Certificates":adminPage==="add-course"?"Add Course":adminPage==="courses"?"Courses":"Dashboard"} | EduConnect Admin`;},[adminPage,submissionsOpen,certificatesOpen]);
