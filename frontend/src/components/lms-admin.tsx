...
function errText(err: unknown) {
  if (err instanceof Error) {
    ...
    return apiErr.issues?.length
      ? `${apiErr.message}: ${apiErr.issues.join(" • ")}`
      : apiErr.message;
  }
...
function QuizBuilder({
  ...
  const [form, setForm] = useState<AdminQuizInput>({
    instructions: "Answer all questions and submit your attempt.",
  ...
  const save = async () => {
    ...
    setError("");
    try {
      const q = await saveAdminQuiz(lessonId, form);
      setQuizId(q.id);
      setStatus(q.status);
      ...
      setError(message);
      toast.error("Unable to save quiz", { description: message });
    } finally {
      setBusy(false);
    }
  };
  const publish = async () => {
    ...
    setError("");
    try {
      const q = await publishAdminQuiz(quizId);
      setStatus(q.status);
      toast.success("Quiz published");
    } catch (e) {
      const message = errText(e);
      setError(message);
      toast.error("Unable to publish quiz", { description: message });
    } finally {
      setBusy(false);
    }
  };
      ...
      <div className="max-w-3xl mx-auto bg-white rounded-2xl p-6 space-y-5">
        <div className="flex justify-between">
          <div>
            ...
            <h2 className="text-[20px] font-bold">Configure assessment</h2>
          </div>
        ...
        <div className="grid grid-cols-3 gap-3">
          <label className="text-[12px]">
            ...
            <input
              ...
              onChange={(e) =>
                setForm({ ...form, passing_percentage: Number(e.target.value) })
              }
          ...
          <label className="text-[12px]">
            ...
            <input
              ...
              onChange={(e) =>
                setForm({ ...form, maximum_attempts: Number(e.target.value) })
              }
              className="w-full p-2 border rounded-lg mt-1"
            />
          </label>
        ...
        {form.questions.map((q, qi) => (
          <div key={qi} className="p-4 border rounded-2xl space-y-3">
            <div className="flex gap-2">
              <input
                ...
                onChange={(e) =>
                  setForm({
                    ...form,
                    questions: form.questions.map((x, i) =>
                      ...
                      i === qi
                        ? {
                            ...
                            question_type: e.target
                              .value as typeof q.question_type,
                          }
                        : x,
                    ),
                  })
                    ...
                    setForm({
                      ...
                      questions: form.questions.map((x, i) =>
                        i !== qi
                          ...
                          : {
                              ...x,
                              options: x.options.map((option, j) => ({
                                ...option,
                                is_correct:
                                  j === oi
                                    ? true
            ...
            <button
              ...
              className="text-[12px] text-[#1B3A6B]"
            >
              + Add option
            </button>
          </div>
        ...
        <div className="flex justify-end gap-2">
          <button
            disabled={busy}
            onClick={save}
            className="px-5 py-2.5 bg-[#1B3A6B] text-white rounded-xl text-[13px]"
          >
            Save Draft
          </button>
          <button
...
const blankAssignment: AdminAssignmentInput = {
  ...
  allowed_file_extensions: ["pdf", "docx", "zip"],
...
function AssignmentBuilder({
  ...
  const [status, setStatus] = useState("draft");
  const [form, setForm] = useState<AdminAssignmentInput>(blankAssignment);
  const [busy, setBusy] = useState(false);
  const [error, setError] = useState("");
  useEffect(() => {
    loadAdminAssignment(lessonId)
      .then((a) => {
        if (a) {
          setId(a.id);
          setStatus(a.status);
          setForm({
            ...
            allowed_file_extensions: a.allowed_file_extensions,
            maximum_file_size_mb: a.maximum_file_size_mb,
            due_at: a.due_at,
  ...
  const save = async () => {
    ...
    setError("");
    try {
      const a = await saveAdminAssignment(lessonId, form);
      setId(a.id);
      setStatus(a.status);
      ...
      const message = errText(e);
      setError(message);
      toast.error("Unable to save assignment", { description: message });
    } finally {
      setBusy(false);
    }
  };
  const publish = async () => {
    ...
    setError("");
    try {
      const a = await publishAdminAssignment(id);
      setStatus(a.status);
      toast.success("Assignment published");
    } catch (e) {
      const message = errText(e);
      setError(message);
      toast.error("Unable to publish assignment", { description: message });
    } finally {
      setBusy(false);
    }
  };
  const toggleType = (value: "file" | "text" | "link") =>
    setForm({
      ...form,
      allowed_submission_types: form.allowed_submission_types.includes(value)
        ? form.allowed_submission_types.filter((x) => x !== value)
        : [...form.allowed_submission_types, value],
    });
  return (
    <div className="fixed inset-0 z-50 bg-[#071326]/70 p-5 overflow-y-auto">
      <div className="max-w-2xl mx-auto bg-white rounded-2xl p-6 space-y-5">
        <div className="flex justify-between">
          <div>
            ...
            <h2 className="text-[20px] font-bold">Configure assignment</h2>
          </div>
          <button onClick={onClose} className="text-[13px]">
            Close
          </button>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
          <label className="text-[12px]">
            ...
            <input
              ...
              onChange={(e) =>
                setForm({ ...form, maximum_marks: Number(e.target.value) })
              }
              className="w-full p-2 border rounded-lg mt-1"
            />
          </label>
          ...
          <label className="text-[12px]">
            ...
            <input
              ...
              min={1}
              max={100}
              value={form.maximum_file_size_mb}
              onChange={(e) =>
                setForm({
                  ...form,
                  maximum_file_size_mb: Number(e.target.value),
                })
              }
              className="w-full p-2 border rounded-lg mt-1"
            />
        ...
        <div>
          <p className="text-[12px] font-semibold mb-2">Allowed submissions</p>
          <div className="flex gap-4">
            {(["file", "text", "link"] as const).map((x) => (
              <label key={x} className="text-[12px] capitalize">
                <input
                  type="checkbox"
                  checked={form.allowed_submission_types.includes(x)}
        ...
        <div className="flex gap-5">
          ...
          <label className="text-[12px]">
            <input
              ...
              onChange={(e) =>
                setForm({ ...form, allow_resubmission: e.target.checked })
              }
              className="mr-1"
            />
            Allow resubmission
...
function SubmissionRow({
  ...
  item: AdminAssignmentSubmission;
  onDone: () => void;
}) {
  const [marks, setMarks] = useState(item.evaluation?.marks_awarded || 0);
  const [decision, setDecision] = useState<
    "passed" | "failed" | "resubmission_required"
  >("passed");
  const [feedback, setFeedback] = useState(item.evaluation?.feedback || "");
  const [busy, setBusy] = useState(false);
  const submit = async () => {
    setBusy(true);
    try {
      await evaluateAssignmentSubmission(item.id, {
        marks_awarded: marks,
        decision,
        feedback,
      });
      ...
      toast.error("Unable to save evaluation", { description: errText(e) });
    } finally {
      setBusy(false);
    }
  };
  return (
    <div className="p-4 border rounded-2xl space-y-3">
      <div className="flex justify-between">
        <div>
          <p className="text-[13px] font-semibold">
            {item.student.full_name} · {item.assignment_title}
          </p>
          <p className="text-[11px] text-[#5A6A8A]">
            {item.course_title} · Attempt {item.attempt_number}
            {item.is_late ? " · Late" : ""}
          </p>
        </div>
      ...
      {item.text_content && (
        <p className="text-[12px] bg-slate-50 p-3 rounded-xl">
          {item.text_content}
        </p>
      )}
      {item.link_url && (
      ...
      {item.has_file && (
        <button
          onClick={() =>
            downloadAssignmentSubmissionFile(
              item.id,
              item.original_file_name || "submission",
            )
          }
          className="text-[12px] text-blue-600 underline"
        >
      ...
      <div className="grid grid-cols-3 gap-2">
        <input
          ...
          className="p-2 border rounded-lg text-[12px]"
        />
        <select
          value={decision}
      ...
      <button
        disabled={busy}
        onClick={submit}
        className="px-4 py-2 bg-[#1B3A6B] text-white rounded-lg text-[12px]"
      >
...
function SubmissionReview({ onClose }: { onClose: () => void }) {
  const [items, setItems] = useState<AdminAssignmentSubmission[]>([]);
  const [error, setError] = useState("");
  const load = () =>
    loadAssignmentSubmissions()
      .then(setItems)
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#071326]/70 p-5 overflow-y-auto">
      <div className="max-w-4xl mx-auto bg-white rounded-2xl p-6 space-y-4">
        <div className="flex justify-between">
          <h2 className="text-[20px] font-bold">Assignment Submissions</h2>
          <button onClick={onClose}>Close</button>
        </div>
        {error && <div className="text-red-600 text-[12px]">{error}</div>}
        {items.map((x) => (
          <SubmissionRow key={x.id} item={x} onDone={load} />
        ))}
        {!items.length && !error && (
          <div className="py-10 text-center text-[#9AA5BE] text-[13px]">
...
function CertificateManagement({ onClose }: { onClose: () => void }) {
  ...
  const load = () =>
    loadAdminCertificates(search, filter)
      .then(setItems)
      .catch((e) => setError(errText(e)));
  useEffect(() => {
  ...
  useEffect(() => {
    if (error) {
      toast.error("Certificate action failed", { description: error });
      setError("");
    }
  }, [error]);
  const revoke = async (item: AdminCertificate) => {
    const reason = window.prompt("Reason for revoking this certificate?");
    if (!reason) return;
    setBusy(item.id);
    setError("");
    try {
      await revokeCertificate(item.id, reason);
  ...
  const reissue = async (item: AdminCertificate) => {
    setBusy(item.id);
    setError("");
    try {
      await reissueCertificate(item.id);
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#071326]/70 p-4 sm:p-5 overflow-y-auto">
      <div className="max-w-5xl mx-auto bg-[#F2F5FC] rounded-2xl overflow-hidden">
        <div className="p-5 bg-[#0F1F3B] text-white flex items-center justify-between">
          ...
          <button
            onClick={onClose}
            className="px-3 py-2 bg-white/10 rounded-xl text-[12px]"
          >
            Close
          </button>
        </div>
        <div className="p-5 space-y-4">
          <div className="bg-white border border-[rgba(27,58,107,0.08)] rounded-2xl p-4 flex flex-col sm:flex-row gap-3">
            <div className="relative flex-1">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2.5 bg-[#F8FAFD] border rounded-xl text-[12px]"
              />
            </div>
            <select
              value={filter}
              onChange={(e) => setFilter(e.target.value)}
              className="px-3 py-2.5 border rounded-xl text-[12px] bg-white"
            >
          ...
          <div className="space-y-3">
            {items.map((item) => (
              <div
                key={item.id}
                className="bg-white border border-[rgba(27,58,107,0.08)] rounded-2xl p-4 flex flex-col md:flex-row md:items-center gap-4"
              >
                <span className="w-10 h-10 shrink-0 rounded-xl bg-[#EBF1FA] flex items-center justify-center">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span
                      className={`px-2 py-0.5 rounded-full text-[9.5px] font-semibold capitalize ${item.status === "issued" ? "bg-emerald-50 text-emerald-700" : item.status === "revoked" ? "bg-red-50 text-red-700" : "bg-slate-100 text-slate-600"}`}
                    >
                      {item.status}
                    </span>
                  </div>
                ...
                <div className="flex gap-2">
                  {item.status === "issued" && (
                    <button
                      ...
                      onClick={() => revoke(item)}
                      className="px-3 py-2 border border-red-200 text-red-700 rounded-xl text-[11px] font-semibold"
                    >
                      Revoke
                    </button>
                  )}
                  {item.status === "revoked" && (
                    <button
                      disabled={busy === item.id}
                      onClick={() => reissue(item)}
                      className="px-3 py-2 bg-[#1B3A6B] text-white rounded-xl text-[11px] font-semibold"
                    >
                      Reissue
                    </button>
                  )}
            ...
            {!items.length && !error && (
              <div className="py-12 text-center text-[13px] text-[#9AA5BE]">
...
export function LMSAdminSection({
  ...
  const [adminPage, setAdminPage] = useState<AdminPage>(() =>
    adminPageFromPath(window.location.pathname),
  );
  ...
  const [codingTestLessonId, setCodingTestLessonId] = useState<string | null>(
    null,
  );
  ...
  const [institutionOpen, setInstitutionOpen] = useState(
    () => window.location.pathname === "/admin/institution",
  );
  const [collegeRequestsOpen, setCollegeRequestsOpen] = useState(
    () => window.location.pathname === "/admin/college-requests",
  );
  const [analytics, setAnalytics] = useState<AdminAnalyticsOverview | null>(
    null,
  );
  const navigateAdmin = (next: AdminPage, path = ADMIN_PAGE_PATH[next]) => {
  ...
  const openCertificates = () => {
    if (window.location.pathname !== "/admin/certificates")
  ...
  const loadDetail = async (courseId: string) => {
    ...
    if (window.location.pathname !== coursePath)
      window.history.pushState({}, "", coursePath);
  ...
  useEffect(() => {
    if (
      ...
      window.history.replaceState({}, "", ADMIN_PAGE_PATH.dashboard);
    const courseId = window.location.pathname.match(
      /^\/admin\/courses\/([^/]+)$/,
    )?.[1];
    if (courseId && courseId !== "new") void loadDetail(courseId);
    const restore = () => {
      const path = window.location.pathname;
      setAdminPage(adminPageFromPath(path));
  ...
  useEffect(() => {
    if (!notice) return;
    toast.success(notice, { id: "admin-operation-success", duration: 3500 });
    setNotice("");
  }, [notice]);
  useEffect(() => {
    if (!error) return;
    toast.error("Action could not be completed", {
      id: "admin-operation-error",
      description: error,
      duration: 6500,
    });
    setError("");
  ...
  const saveCourse = async () => {
    ...
    try {
      ...
      if (creating) navigateAdmin("courses");
  ...
  const publishSelected = async () => {
    ...
    setSubmitting(true);
    try {
      const result = await publishCourse(selectedId);
      setDetail(result);
  ...
  const archiveSelected = async () => {
    ...
    setSubmitting(true);
    try {
      const result = await archiveCourse(selectedId);
      setDetail(result);
  ...
  const restoreSelected = async () => {
    ...
    setSubmitting(true);
    try {
      const result = await restoreCourse(selectedId);
      setDetail(result);
      setNotice("Course restored.");
      await loadList(page);
    } catch (err) {
      setError(errText(err));
    } finally {
  ...
  const addSection = async () => {
    if (!selectedId || !sectionTitle.trim()) return;
    setSubmitting(true);
    try {
      await createSection(selectedId, { title: sectionTitle.trim() });
      setSectionTitle("");
  ...
  const deleteSectionId = async (sectionId: string) => {
  ...
  const moveSection = async (sectionId: string, direction: -1 | 1) => {
    if (!detail) return;
    const ids = detail.sections.map((section) => section.id);
    ...
    setSubmitting(true);
    try {
      const result = await reorderSections(detail.id, { ids });
      setDetail(result);
    } catch (err) {
      setError(errText(err));
    } finally {
  ...
  const startLesson = (
    sectionId: string,
    lesson?: AdminCourseDetail["sections"][number]["lessons"][number],
  ) => {
  ...
  const submitLesson = async () => {
    ...
    try {
      const payload = {
        ...
        youtube_id: lessonForm.youtube_id?.trim() || null,
        article_content: lessonForm.article_content?.trim() || null,
      } satisfies LessonInput;
      if (editingLessonId) {
        await updateLesson(editingLessonId, payload);
        setNotice("Lesson updated.");
      } else {
        await createLesson(lessonSectionId, payload);
  ...
  const deleteLessonId = async (lessonId: string) => {
  ...
  const moveLesson = async (
    ...
    setSubmitting(true);
    try {
      const result = await reorderLessons(sectionId, { ids });
      setDetail((current) =>
        current
          ? {
              ...current,
  ...
  const stats = [
    ...
    },
    {
      ...
      value: analytics?.summary.active_learners ?? "—",
      icon: Activity,
    },
    {
      label: "Enrollments",
      value: analytics?.summary.total_enrollments ?? "—",
      icon: BookOpen,
    },
    {
      label: "Completion rate",
      value: analytics ? `${analytics.summary.course_completion_rate}%` : "—",
      icon: CheckCircle2,
    },
  ];
  const openNewCourse = () => {
  ...
  const navigation = [
    ...
    },
    {
      ...
      label: "Add Course",
      description: "Create systematically",
      icon: Plus,
    },
  ];
  const adminName =
    adminProfile?.email
      .split("@")[0]
      .replace(/[._-]+/g, " ")
      .replace(/\b\w/g, (value) => value.toUpperCase()) || "LMS Admin";
  const adminInitials =
    adminName
      .split(" ")
      .slice(0, 2)
  ...
  const handleLogout = async () => {
    ...
    try {
      await (onLogout?.() ?? onBack?.());
      toast.success("Signed out successfully", { id: "admin-logout" });
    } catch (error) {
  ...
  if (collegeRequestsOpen) {
    return (
      <InstitutionApprovalPanel
        onBack={() => {
          window.history.pushState({}, "", ADMIN_PAGE_PATH.dashboard);
  ...
  return (
    <div
      className="min-h-screen bg-[#F2F5FC] text-[#0F1C3F]"
    ...
    >
      <Toaster
        ...
        visibleToasts={4}
        toastOptions={{
          className: "!rounded-2xl !border !shadow-xl !font-[var(--font-sans)]",
      ...
      {certificatesOpen && <CertificateManagement onClose={closeOverlay} />}
      {institutionOpen && <InstitutionManagement onClose={closeOverlay} />}
      <header className="sticky top-0 z-20 bg-[#0F1F3B] text-white border-b border-white/5">
        <div className="px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-[13px] font-semibold">EduConnect</p>
              <p className="text-[10px] uppercase tracking-[0.2em] text-white/45 mt-1">
                Admin LMS
              </p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <button
              ...
              className="hidden sm:flex px-3 py-2 rounded-xl bg-[#D97706] text-[12px] font-semibold items-center gap-2"
            >
              <Plus size={14} />
              New Course
            </button>
            <div className="relative">
              <button
                ...
                className="flex items-center gap-2.5 pl-1.5 pr-2 py-1.5 rounded-xl hover:bg-white/10 transition-colors"
              >
                <span className="w-9 h-9 rounded-xl bg-gradient-to-br from-[#D97706] to-[#F59E0B] text-white flex items-center justify-center text-[11px] font-bold shadow-sm">
                  {adminInitials}
                </span>
                <span className="hidden md:block text-left max-w-[150px]">
                  <b className="block text-[11.5px] truncate">{adminName}</b>
                  <small className="block text-[9.5px] text-white/45 truncate">
                    Administrator
                  </small>
                </span>
                <ChevronDown
                  size={13}
                  className={`text-white/55 transition-transform ${profileOpen ? "rotate-180" : ""}`}
                />
              </button>
              {profileOpen && (
                <>
                  ...
                  />
                  <div className="absolute right-0 top-[calc(100%+10px)] z-30 w-[290px] bg-white text-[#0F1C3F] rounded-2xl border border-slate-200 shadow-2xl overflow-hidden">
                    <div className="p-4 bg-gradient-to-br from-[#F8FAFD] to-[#EBF1FA]">
                      <div className="flex items-center gap-3">
                        <span className="w-11 h-11 rounded-xl bg-[#1B3A6B] text-white flex items-center justify-center text-[13px] font-bold">
                          {adminInitials}
                        </span>
                        <div className="min-w-0">
                          ...
                          <p className="text-[10.5px] text-[#5A6A8A] truncate">
                            LMS Administrator
                          </p>
                        </div>
                      </div>
                    </div>
                    <div className="px-4 py-3 space-y-2 border-y">
                      <p className="flex items-center gap-2 text-[11px] text-[#5A6A8A]">
                        <Mail size={13} className="shrink-0" />
                        <span className="truncate">
                          {adminProfile?.email || "Loading profile…"}
                        </span>
                      </p>
                      {adminProfile?.mobile && (
                        <p className="flex items-center gap-2 text-[11px] text-[#5A6A8A]">
                      ...
                      <p className="flex items-center gap-2 text-[10.5px] text-emerald-700">
                    ...
                    <div className="p-2">
                      <button
                        ...
                        className="w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left text-red-600 hover:bg-red-50 disabled:opacity-50"
                      >
                        <span className="w-8 h-8 rounded-lg bg-red-50 flex items-center justify-center">
                          <LogOut size={14} />
                        </span>
                        <span>
                          <b className="block text-[11.5px]">Logout</b>
                          <small className="text-[9.5px] text-red-400">
      ...
      <div className="flex min-h-[calc(100vh-65px)]">
        <aside
          className={`${sidebarOpen ? "block" : "hidden"} lg:block fixed lg:sticky top-[65px] z-10 lg:z-auto w-[260px] h-[calc(100vh-65px)] shrink-0 bg-white border-r border-[rgba(27,58,107,0.08)] p-4`}
        >
          <div className="mb-5 p-4 rounded-2xl bg-[#1B3A6B] text-white">
            <p className="text-[10px] uppercase tracking-widest text-white/50">
              Administration
            </p>
          ...
          <nav className="space-y-1">
              ...
              return (
                <button
                  ...
                  onClick={() =>
                    item.key === "add-course"
                      ? openNewCourse()
                      : navigateAdmin(item.key)
                  }
                  className={`w-full flex items-center gap-3 p-3 rounded-xl text-left ${active ? "bg-[#EBF1FA] text-[#1B3A6B]" : "text-[#5A6A8A] hover:bg-[#F8FAFD]"}`}
                >
                  <span
                    className={`w-9 h-9 rounded-xl flex items-center justify-center ${active ? "bg-[#1B3A6B] text-white" : "bg-[#EFF2FA]"}`}
                  >
                    <Icon size={16} />
                  </span>
                  <span>
                    <b className="block text-[12.5px]">{item.label}</b>
                    <small className="text-[10.5px] text-[#9AA5BE]">
                      {item.description}
                    </small>
                  </span>
                </button>
            ...
            >
              ...
              <span>
                <b className="block text-[12.5px]">Submissions</b>
                <small className="text-[10.5px] text-[#9AA5BE]">
                  Review assignments
                </small>
              </span>
            ...
            >
              ...
              <span>
                <b className="block text-[12.5px]">Certificates</b>
                <small className="text-[10.5px] text-[#9AA5BE]">
                  Manage credentials
                </small>
              </span>
            ...
            >
              ...
              <span>
                <b className="block text-[12.5px]">College Requests</b>
                <small className="text-[10.5px] text-[#9AA5BE]">
        ...
        <div className="flex-1 min-w-0 px-5 py-5 space-y-5">
          {adminPage === "dashboard" && (
            <>
              <div>
                <p className="text-[11px] uppercase tracking-widest text-[#9AA5BE]">
                  Admin workspace
                </p>
                <h1
                ...
                <p className="text-[12.5px] text-[#5A6A8A] mt-1">
                  Monitor learning performance and manage key LMS operations.
                </p>
              </div>
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
                {stats.map((stat) => (
                  <div
                    key={stat.label}
                    className="bg-white rounded-2xl border border-[rgba(27,58,107,0.08)] p-4 shadow-sm"
                  >
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-[12px] text-[#5A6A8A]">
                        {stat.label}
                      </span>
                      <stat.icon size={15} className="text-[#1B3A6B]" />
                    </div>
              ...
              {analytics && (
                <div className="grid xl:grid-cols-[1fr_1.5fr] gap-4">
                  <div className="bg-white rounded-2xl border border-[rgba(27,58,107,0.08)] p-5 shadow-sm">
                    <div className="flex items-center gap-3">
                      <span className="w-9 h-9 rounded-xl bg-[#EBF1FA] flex items-center justify-center">
                        <BarChart3 size={16} className="text-[#1B3A6B]" />
                      </span>
                      <div>
                        <h2 className="text-[14px] font-semibold text-[#0F1C3F]">
                          Learning health
                        </h2>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mt-5">
                      {[
                        {
                          value: `${analytics.summary.average_quiz_score}%`,
                          label: "Quiz average",
                        },
                        {
                          value:
                            analytics.summary.pending_assignment_evaluations,
                          label: "Pending reviews",
                        },
                        {
                          value: analytics.summary.video_learning_minutes,
                          label: "Video minutes",
                        },
                      ].map((item) => (
                        <div
                          key={item.label}
                          className="p-3 bg-[#F8FAFD] border border-[rgba(27,58,107,0.08)] rounded-xl text-center"
                        >
                          <p className="text-[20px] font-bold text-[#1B3A6B]">
                            {item.value}
                          </p>
                          <p className="text-[10px] text-[#5A6A8A] mt-0.5">
                            {item.label}
                          </p>
                        </div>
                      ))}
                  ...
                  <div className="bg-white rounded-2xl border border-[rgba(27,58,107,0.08)] p-5 shadow-sm">
                    <div className="flex items-start justify-between gap-3">
                      ...
                      <span className="text-[10.5px] text-[#5A6A8A] bg-[#F4F7FC] px-2.5 py-1 rounded-full">
                        Top enrollments
                      </span>
                    </div>
                    <div className="grid md:grid-cols-2 gap-x-5 gap-y-4 mt-4">
                      {analytics.course_performance
                        .slice(0, 4)
                        .map((course) => (
                          <div key={course.course_id} className="min-w-0">
                            <div className="flex justify-between gap-3 text-[11px]">
                              <span className="truncate text-[#0F1C3F] font-medium">
                                {course.title}
                              </span>
                            ...
                            <div className="h-1.5 bg-[#E8ECF5] rounded-full mt-2 overflow-hidden">
                              <div
                                className="h-full bg-[#1B3A6B] rounded-full"
                                style={{ width: `${course.average_progress}%` }}
                              />
                            </div>
                            <p className="text-[9.5px] text-[#9AA5BE] mt-1.5">
              ...
              <div className="grid sm:grid-cols-3 gap-3">
                <button
                  onClick={() => navigateAdmin("courses")}
                  className="p-5 bg-white border rounded-2xl text-left"
                >
                  <BookOpen size={18} className="text-[#1B3A6B]" />
                  <b className="block text-[13px] mt-3">Manage Courses</b>
                  <span className="text-[11px] text-[#9AA5BE]">
                    {total} courses available
                  </span>
                </button>
                <button
                  onClick={openNewCourse}
                  className="p-5 bg-[#1B3A6B] text-white rounded-2xl text-left"
                >
                  <Plus size={18} />
                  <b className="block text-[13px] mt-3">Create Course</b>
                ...
                >
                  ...
                  <b className="block text-[13px] mt-3">Review Work</b>
                  <span className="text-[11px] text-[#9AA5BE]">
                    Pending evaluations
                  </span>
                </button>
          ...
          <div
            className={`${adminPage === "dashboard" ? "hidden" : "flex"} flex-col lg:flex-row gap-4`}
          >
            <section
              className={`${adminPage === "add-course" ? "hidden" : "block"} lg:w-[48%] space-y-4`}
            >
                ...
                <div className="flex gap-2 flex-col md:flex-row md:items-center">
                  <div className="relative flex-1">
                    ...
                    <input
                      ...
                      className="w-full pl-8 pr-3 py-2.5 bg-[#F8FAFD] border border-[rgba(27,58,107,0.12)] rounded-xl text-[13px] outline-none"
                    />
                  </div>
                    ...
                    {(["all", "published", "draft", "archived"] as const).map(
                      (status) => (
                        <button
                          ...
                          className={`px-3 py-2 rounded-xl text-[12px] font-medium capitalize ${statusFilter === status ? "bg-[#1B3A6B] text-white" : "bg-white text-[#5A6A8A]"}`}
                        >
                          {status}
                        </button>
                      ),
                  ...
                  <select
                    ...
                    className="px-3 py-2 rounded-xl border border-[rgba(27,58,107,0.12)] text-[12px] bg-white outline-none"
                  >
              ...
              <div className="bg-white rounded-2xl border border-[rgba(27,58,107,0.08)] shadow-sm overflow-hidden">
                {loading ? (
                  <div className="p-10 flex items-center justify-center gap-2 text-[#5A6A8A]">
                    <Loader2 size={16} className="animate-spin" />
                    Loading…
                  </div>
                ) : items.length === 0 ? (
                  <div className="p-10 text-center text-[#5A6A8A]">
                    <BookOpen
                      size={22}
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-[rgba(27,58,107,0.08)] bg-[#F8FAFD]">
                        <th className="px-4 py-3 text-left text-[11px] uppercase tracking-wide text-[#9AA5BE]">
                          Course
                        </th>
                        <th className="px-4 py-3 text-left text-[11px] uppercase tracking-wide text-[#9AA5BE]">
                          Status
                    ...
                    <tbody>
                      {items.map((course) => (
                        <tr
                          key={course.id}
                          className="border-b border-[rgba(27,58,107,0.05)] hover:bg-[#FAFBFE]"
                        >
                          ...
                          <td className="px-4 py-4">
                            <span
                              className={`inline-flex px-2.5 py-1 rounded-full text-[11px] font-semibold border capitalize ${STATUS_BADGE[course.status]}`}
                            >
                              {course.status}
                            </span>
                          </td>
                          <td className="px-4 py-4">
                            <div className="flex justify-end gap-1">
                              <button
                                onClick={() => void loadDetail(course.id)}
                                className="p-2 rounded-lg hover:bg-[#EBF1FA]"
                              >
                                <Edit2 size={14} />
                              </button>
                            </div>
              ...
              <div className="flex items-center justify-between text-[12px] text-[#5A6A8A]">
                <span>
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="px-3 py-2 rounded-xl border border-[rgba(27,58,107,0.12)] disabled:opacity-40 flex items-center gap-1"
                  >
                    <ChevronLeft size={14} />
                    Prev
                  </button>
            ...
            <section
              className={`${adminPage === "add-course" ? "w-full max-w-4xl mx-auto" : "lg:w-[52%]"} bg-white rounded-2xl border border-[rgba(27,58,107,0.08)] shadow-sm overflow-hidden`}
            >
              {!editorOpen ? (
                <div className="p-8 text-center text-[#5A6A8A]">
                  <Layers size={24} className="mx-auto mb-3 text-[#CBD5E1]" />
                  Select a course or create a new one to open the curriculum
                  builder.
                </div>
              ) : !detail ? (
                <div className="p-6 space-y-6">
                  <div>
                    <span
                      className={`inline-flex px-2.5 py-1 rounded-full text-[11px] font-semibold border ${STATUS_BADGE.draft}`}
                    >
                      New draft
                    </span>
                    <h2
                    ...
                    <p className="text-[13px] text-[#5A6A8A] mt-1">
                      Complete three short steps. Curriculum can be added after
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {[
                      ...
                      <div
                        key={step.n}
                        className={`p-3 rounded-xl border ${courseStep === step.n ? "bg-[#EBF1FA] border-[#1B3A6B]" : courseStep > step.n ? "bg-emerald-50 border-emerald-200" : "bg-[#F8FAFD] border-slate-200"}`}
                      >
                        <span
                          className={`w-6 h-6 rounded-full inline-flex items-center justify-center text-[10px] font-bold ${courseStep >= step.n ? "bg-[#1B3A6B] text-white" : "bg-slate-200 text-slate-500"}`}
                        >
                          {courseStep > step.n ? (
                        ...
                        <p className="text-[11.5px] font-semibold mt-2">
                          {step.label}
                        </p>
                      </div>
                    ))}
                    ...
                    <div className="space-y-4">
                      <label className="block text-[12px] font-semibold">
                        ...
                        <input
                          ...
                          placeholder="e.g. Backend APIs with Node.js"
                          className="w-full mt-1.5 px-3 py-3 rounded-xl border border-[rgba(27,58,107,0.12)] bg-[#F8FAFD] outline-none"
                        />
                      </label>
                      <label className="block text-[12px] font-semibold">
                        ...
                        <textarea
                          ...
                          rows={5}
                          className="w-full mt-1.5 px-3 py-3 rounded-xl border border-[rgba(27,58,107,0.12)] bg-[#F8FAFD] outline-none resize-none"
                  ...
                  {courseStep === 2 && (
                    <div className="grid md:grid-cols-2 gap-4">
                      ...
                      <label className="text-[12px] font-semibold">
                        ...
                        <input
                          value={joinSkills(courseForm.skills)}
                          onChange={(e) =>
                            setCourseForm((value) => ({
                              ...value,
                              skills: splitSkills(e.target.value),
                            }))
                          }
                          placeholder="Node.js, REST, PostgreSQL"
                          className="w-full mt-1.5 px-3 py-3 rounded-xl border bg-[#F8FAFD]"
                        />
                      </label>
                  ...
                  {courseStep === 3 && (
                    <div className="rounded-2xl border bg-[#F8FAFD] p-5 space-y-4">
                      <div className="flex items-start justify-between gap-4">
                        <div>
                          <p className="text-[10px] uppercase tracking-widest text-[#9AA5BE]">
                            Draft preview
                          </p>
                          <h3 className="text-[18px] font-bold mt-1">
                            {courseForm.title}
                          </h3>
                          <p className="text-[12px] text-[#5A6A8A] mt-2">
                            {courseForm.description}
                          </p>
                        </div>
                        <span className="px-3 py-1.5 bg-slate-100 rounded-full text-[10px] font-semibold">
                          {courseForm.level}
                        </span>
                      </div>
                      <div className="grid sm:grid-cols-3 gap-3">
                        {[
                          {
                            label: "Duration",
                            value: `${courseForm.duration_hours} hours`,
                          },
                          {
                            label: "Instructor",
                            value:
                              courseForm.instructor_name || "Not specified",
                          },
                          { label: "Skills", value: courseForm.skills.length },
                        ].map((x) => (
                          <div
                            key={x.label}
                            className="bg-white p-3 rounded-xl border"
                          >
                            <p className="text-[10px] text-[#9AA5BE]">
                              {x.label}
                            </p>
                      ...
                      <p className="text-[11px] text-[#5A6A8A]">
                        Course draft create hone ke baad Curriculum Builder
                  ...
                  <div className="flex justify-between border-t pt-5">
                    <button
                      disabled={courseStep === 1 || submitting}
                      onClick={() => setCourseStep((x) => Math.max(1, x - 1))}
                      className="px-4 py-2.5 rounded-xl border text-[12px] font-semibold disabled:opacity-40"
                    >
                      Previous
                    </button>
                    {courseStep < 3 ? (
                        ...
                        disabled={
                          ...
                          (courseStep === 2 &&
                            (!courseForm.duration_hours ||
                              !courseForm.skills.length))
                        }
                        onClick={() => setCourseStep((x) => Math.min(3, x + 1))}
                        className="px-5 py-2.5 rounded-xl bg-[#1B3A6B] text-white text-[12px] font-semibold disabled:opacity-40"
                      >
                        Continue
                      </button>
                    ) : (
                      <button
                        ...
                        className="px-5 py-2.5 rounded-xl bg-[#1B3A6B] text-white text-[12px] font-semibold flex items-center gap-2 disabled:opacity-60"
                      >
                        <Save size={14} />
                        {submitting
                          ? "Creating…"
                          : "Create Draft & Add Curriculum"}
                      </button>
                    )}
                  </div>
                </div>
                ...
                <div className="p-5 space-y-5">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <span
                        className={`inline-flex px-2.5 py-1 rounded-full text-[11px] font-semibold border capitalize ${STATUS_BADGE[detail.status]}`}
                      >
                        {detail.status}
                      </span>
                      <h2
                  ...
                  <textarea
                    ...
                    rows={4}
                    className="w-full px-3 py-2.5 rounded-xl border border-[rgba(27,58,107,0.12)] bg-[#F8FAFD] outline-none resize-none"
                  ...
                  <div className="flex gap-2 flex-wrap">
                    <button
                      disabled={submitting}
                      onClick={() => void saveCourse()}
                      className="px-4 py-2.5 rounded-xl bg-[#1B3A6B] text-white text-[13px] font-semibold flex items-center gap-2 disabled:opacity-60"
                    >
                      <Save size={14} />
                      Save
                    </button>
                    <button
                      ...
                      className="px-4 py-2.5 rounded-xl border border-[rgba(27,58,107,0.12)] text-[13px] font-semibold disabled:opacity-60"
                    >
                      Publish
                    </button>
                    <button
                    ...
                    <button
                      ...
                      className="px-4 py-2.5 rounded-xl border border-red-200 text-red-600 text-[13px] font-semibold disabled:opacity-60"
                    >
                      Delete
                    </button>
                  </div>
                  <div className="rounded-2xl border border-[rgba(27,58,107,0.08)] bg-[#FAFBFE] p-4">
                    <div className="flex items-center justify-between mb-3">
                      <div>
                        ...
                        <p className="text-[12px] text-[#5A6A8A]">
                          {detail.publication_readiness.is_ready
                            ? "Ready to publish"
                    ...
                    <div className="space-y-2">
                      {detail.publication_readiness.issues.length === 0 ? (
                        <p className="text-[13px] text-emerald-700">
                          No blocking issues.
                        </p>
                      ) : (
                        detail.publication_readiness.issues.map((issue) => (
                          <div
                            key={issue}
                            className="flex items-start gap-2 text-[12.5px] text-amber-700"
                          >
                  ...
                  <div className="space-y-3">
                    <div className="grid md:grid-cols-[1fr_auto] gap-2">
                      <input
                        ...
                        className="w-full px-3 py-2.5 rounded-xl border border-[rgba(27,58,107,0.12)] bg-white outline-none"
                      />
                      <button
                        disabled={submitting || !sectionTitle.trim()}
                    ...
                    {detail.sections.map((section, index) => (
                      <div
                        key={section.id}
                        className="rounded-2xl border border-[rgba(27,58,107,0.08)] bg-white overflow-hidden"
                      >
                        <div className="px-4 py-3 flex items-start justify-between gap-3 border-b border-[rgba(27,58,107,0.05)] bg-[#FAFBFE]">
                          <div>
                            {editingSectionId === section.id ? (
                              <input
                                ...
                                className="px-3 py-2 rounded-xl border border-[rgba(27,58,107,0.12)] bg-white outline-none"
                              />
                            ) : (
                              <>
                                <p className="text-[13.5px] font-semibold">
                          ...
                          <div className="flex items-center gap-1">
                            {editingSectionId === section.id ? (
                              <button
                                onClick={() => void updateSectionTitle()}
                                className="p-2 rounded-lg hover:bg-[#EBF1FA]"
                              >
                                <Save size={14} />
                              </button>
                            ...
                            <button
                              onClick={() => void deleteSectionId(section.id)}
                        ...
                        <div className="p-4 space-y-3">
                          {section.lessons.map((lesson) => {
                            ...
                            return (
                              <div
                                key={lesson.id}
                                className="rounded-xl border border-[rgba(27,58,107,0.08)] bg-[#FCFDFF] p-3"
                              >
                                <div className="flex items-start justify-between gap-3">
                                  <div>
                                    <div className="inline-flex items-center gap-1.5 px-2 py-1 rounded-full border text-[11px] font-semibold bg-slate-50 text-slate-700 border-slate-100">
                                      <meta.icon size={11} />
                                      {meta.label}
                                    </div>
                                    ...
                                    {lesson.lesson_type === "quiz" && (
                                      <button
                                        ...
                                        className="mt-2 px-3 py-1.5 bg-purple-50 text-purple-700 rounded-lg text-[11px] font-semibold"
                                      >
                                        Configure Quiz
                                      </button>
                                    )}
                                    {lesson.lesson_type === "assignment" && (
                                      <button
                                        onClick={() =>
                                          setAssignmentLessonId(lesson.id)
                                        }
                          ...
                          {lessonSectionId === section.id ? (
                            <div className="rounded-xl border border-dashed border-[rgba(27,58,107,0.2)] bg-[#F8FAFD] p-4 space-y-3">
                              <div className="grid md:grid-cols-2 gap-3">
                                <input
                                ...
                                <select
                                  ...
                                  className="w-full px-3 py-2.5 rounded-xl border border-[rgba(27,58,107,0.12)] bg-white outline-none"
                                >
                                  {Object.entries(LESSON_META).map(
                                    ([value, meta]) => (
                              ...
                              <textarea
                                ...
                                className="w-full px-3 py-2.5 rounded-xl border border-[rgba(27,58,107,0.12)] bg-white outline-none resize-none"
                              />
                              <label className="flex items-center gap-2 text-[12px] text-[#5A6A8A]">
                                <input
                                  type="checkbox"
                              ...
                              <div className="flex gap-2">
                                ...
                                <button
                                  ...
                                  className="px-4 py-2.5 rounded-xl border border-[rgba(27,58,107,0.12)] text-[13px] font-semibold"
                                >
                                  Cancel
                                </button>
                              </div>
                            ...
                            <button
                              onClick={() => startLesson(section.id)}
                              className="w-full py-3 rounded-xl border border-dashed border-[rgba(27,58,107,0.18)] text-[13px] font-semibold text-[#1B3A6B]"
                            >
                              Add lesson
                            </button>
                          )}
