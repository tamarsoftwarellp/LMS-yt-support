...
const input =
  "w-full px-3.5 py-2.5 bg-[#EFF2FA] border-[1.5px] border-transparent rounded-[10px] text-[13px] text-[#0F1C3F] outline-none focus:border-[#1B3A6B] focus:bg-white";
...
const allowText = (value: string) => /^[A-Za-z\s.,'()&+\-/:;#]*$/.test(value);

const allowTextWithNumbers = (value: string) =>
  /^[A-Za-z0-9\s.,'()&+\-/:;#]*$/.test(value);

const allowNumber = (value: string) => /^\d*$/.test(value);
  ...
  return (
    <section className="space-y-3">
      <div className="flex items-center justify-between">
        <h3 className="text-[14px] font-semibold text-[#0F1C3F]">{label}</h3>
        <button
          type="button"
      ...
      {items.map((x, i) => (
        <div
          key={i}
          className="p-4 rounded-xl border border-[#E3E8F2] bg-[#FBFCFE] space-y-3"
        >
          <div className="grid sm:grid-cols-2 gap-3">
            <input
              ...
              value={x.title}
              placeholder={
                label === "Education" ? "Degree / qualification" : "Title"
              }
              onChange={(e) => {
                const value = e.target.value;

                if (allowTextWithNumbers(value)) {
                  set(i, "title", value);
                }
              }}
            />
...
export function AtsResumeBuilder() {
  ...
  const load = async () => {
    const [builder, list] = await Promise.all([
      loadResumeBuilder(),
      listGeneratedResumes(),
    ]);
    setData(builder);
    setTarget(
      (current) =>
        current || builder.profile.headline || list[0]?.target_role || "",
    );
    setVersions(list);
    if (list[0]) setCurrent(await getGeneratedResume(list[0].id));
  };
  useEffect(() => {
  ...
  const validateProfile = () => {
    ...
    if (headline && !/^[A-Za-z0-9\s.+#&/()'-]+$/.test(headline)) {
      setError("Professional headline contains invalid characters.");
      return false;
    }
  ...
  const generate = async () => {
    ...
    try {
      ...
      setMessage(
        "ATS-friendly resume generated. Review and edit before downloading.",
      );
    } catch (e) {
  ...
  const updateContent = (key: keyof ResumeContent, value: unknown) =>
    setCurrent((x) =>
      x && x.content ? { ...x, content: { ...x.content, [key]: value } } : x,
    );
  const saveGenerated = async () => {
    ...
    setError("");
    try {
      const result = await updateGeneratedResume(
        current.id,
        current.title,
        current.content,
      );
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-[13px] text-[#5A6A8A] mt-1">
          Build a fact-safe resume from your student profile, skills, projects,
      ...
      <div className="p-3 rounded-xl bg-emerald-50 border border-emerald-200 flex items-center gap-2 text-[11.5px] text-emerald-800">
        ...
        Courses you complete on the LMS are automatically added to your
        ...
        <div className="p-4 rounded-xl bg-blue-50 border border-blue-200 flex items-start gap-3 text-[11.5px] text-blue-900">
          ...
          <div>
            <p className="font-semibold">
              {" "}
              Data imported from {data.auto.uploaded_resume.file_name}{" "}
            </p>
            <p className="mt-0.5 text-blue-700">
              {data.auto.uploaded_resume.imported_entries} section entries and{" "}
              {data.auto.uploaded_resume.imported_fields.length} profile fields
              were auto-filled. Review and edit them before generating.
            </p>
          </div>
        </div>
      )}
      ...
      <textarea
        ...
        placeholder="About you (facts only; Groq will polish the wording)"
        onChange={(e) => {
          const value = e.target.value;
      ...
      <Entries
        label="Education"
        items={data.profile.educations}
        onChange={(x) => profile("educations", x)}
      />
      <Entries
        label="Experience"
        items={data.profile.experiences}
        onChange={(x) => profile("experiences", x)}
      />
      <Entries
        label="Projects"
        items={data.profile.projects}
        onChange={(x) => profile("projects", x)}
      />
      <Entries
      ...
      <div className="grid sm:grid-cols-2 gap-3">
        <textarea
          ...
          onChange={(e) => {
            ...
            if (allowTextWithNumbers(value)) {
              profile("achievements", value.split("\n").filter(Boolean));
            }
          }}
        />
        <textarea
      ...
      <div className="p-5 rounded-2xl bg-[#1B3A6B] text-white">
        ...
        <p className="text-[10.5px] text-white/55 mt-2">
          Your professional headline is used automatically. You can change the
          target role here. AI improves wording only and will not invent facts.
        </p>
      </div>
      {versions.length > 0 && (
        <div className="flex gap-2 overflow-x-auto">
          {versions.map((v) => (
            <button
              key={v.id}
              onClick={() => getGeneratedResume(v.id).then(setCurrent)}
              className={`px-3 py-2 rounded-lg text-[11px] whitespace-nowrap ${current?.id === v.id ? "bg-[#1B3A6B] text-white" : "bg-[#EFF2FA] text-[#5A6A8A]"}`}
            >
              Version {v.version} · {v.target_role}
            </button>
          ))}
        </div>
      )}
      {current?.content && (
        <div className="border border-[#E3E8F2] rounded-2xl overflow-hidden">
          <div className="p-5 bg-[#F8FAFD] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 px-4 py-2.5 bg-[#1B3A6B] text-white rounded-xl text-[12px] font-semibold"
            >
              <Download size={14} />
              Download PDF
            </button>
          ...
          <div className="p-5 space-y-4">
            <label className="block text-[11px] font-semibold text-[#5A6A8A]">
              ...
              <textarea
                ...
                onChange={(e) => {
                  ...
                  if (allowTextWithNumbers(value)) {
                    updateContent("professional_summary", value);
                  }
                }}
              />
            </label>
            ...
            >
              {busy === "resume"
                ? "Saving…"
                : "Save Edits & Recalculate ATS Score"}
            </button>
          </div>
        </div>
      )}
