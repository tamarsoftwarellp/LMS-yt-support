...
function SkillsView({ onReady }: { onReady: () => void }) {
  ...
  const add = () => {
    ...
    const months = Number(draft.experience_months);
...
function GoalView({ onReady }: { onReady: () => void }) {
  ...
  const save = async () => {
    ...
    const preferredDomain = goal.preferred_domain.trim();
    const weeklyHours = Number(goal.weekly_learning_hours);
    ...
    if (!Number.isInteger(weeklyHours) || weeklyHours < 1 || weeklyHours > 60) {
      setError("Weekly Learning Hours must be between 1 and 60.");
      return;
    }
    ...
    try {
      const updatedGoal = await saveGoal({
        ...goal,
    ...
    <div className="space-y-6">
          ...
          <input
            ...
            onChange={(e) => {
              ...
              if (
                ...
                setGoal({
                  ...goal,
                  weekly_learning_hours: numberValue,
                });
              }
            }}
          />
      ...
      <div className="flex justify-end">
        <button
          disabled={
            saving ||
            !goal.target_role.trim() ||
            !goal.preferred_domain.trim() ||
            goal.weekly_learning_hours < 1 ||
            goal.weekly_learning_hours > 60
          }
          onClick={save}
          className="px-5 py-2.5 bg-[#1B3A6B] text-white rounded-[10px] text-[13px] font-semibold disabled:opacity-50"
        >
...
export function StudentCareerPortal({
  ...
  const isSectionLocked = (key: Section) => {
    switch (key) {
      case "dashboard":
        return false;
      ...
      case "resume":
        return !ready.skills || !ready.goal;

      case "ats-resume":
        return !ready.skills || !ready.goal || !ready.resume;
  ...
  const lmsLocked = !ready.skills || !ready.goal || !ready.resume;
  ...
  useEffect(() => {
    ...
    const restore = () => {
      const requestedSection = sectionFromPath(window.location.pathname);

      if (isSectionLocked(requestedSection)) {
        toast.error("Please complete the previous step first.");

        window.history.replaceState({}, "", SECTION_PATH.skills);
          ...
          <div className="flex items-center gap-3">
            <button
              ...
              onClick={() => {
                if (lmsLocked) {
                  toast.error(
                    "Please complete Skills, Career Goal and Resume first.",
                  );
                  return;
                }
              ...
              title={
                lmsLocked
                  ? "Complete Skills, Career Goal and Resume first"
                  : "Go to LMS"
              }
            ...
            >
              {lmsLocked ? <Lock size={13} /> : <ExternalLink size={13} />}
              Go to LMS
            </button>
              ...
              {profileOpen && (
                <div className="absolute right-0 mt-2 w-64 rounded-2xl border border-slate-200 bg-white p-2 shadow-2xl">
                  ...
                  <button
                    ...
                    className={`w-full flex sm:hidden mt-1 items-center gap-2.5 px-3 py-2.5 rounded-xl text-[12px] font-semibold transition-all ${
                      lmsLocked
                        ? "text-[#9AA5BE] opacity-50 cursor-not-allowed"
                        : "text-[#1B3A6B] hover:bg-[#EBF1FA]"
                    }`}
                  >
                    {lmsLocked ? (
                      <Lock size={14} />
              ...
              <nav className="p-2.5">
                    ...
                    item.key === "skills"
                      ...
                      : item.key === "goal"
                        ...
                        : item.key === "resume"
                          ? ready.resume
                          : false;
                  const locked = isSectionLocked(item.key);
                  return (
                    <button
                      key={item.key}
                      disabled={locked}
                      ...
                      title={
                        locked ? "Complete the previous step first" : item.desc
                      }
                    ...
                    >
                      ...
                      {locked ? (
                        <ShieldCheck size={13} className="text-[#9AA5BE]" />
                      ) : complete ? (
                ...
                <button
                  ...
                  title={
                    lmsLocked
                      ? "Complete Skills, Career Goal and Resume first"
                      : "Access & learn your courses"
                  }
                  className={`w-full flex items-center gap-3 px-3 py-3 rounded-xl text-left mt-2 border-t border-[--border] pt-4 transition-all ${
                    lmsLocked
                ...
                >
                  <div
                    className={`w-8 h-8 rounded-lg flex items-center justify-center ${
                      lmsLocked ? "bg-slate-100" : "bg-emerald-50"
                    }`}
                  >
                    <ExternalLink
                      size={14}
                      className={
                        lmsLocked ? "text-[#9AA5BE]" : "text-emerald-600"
                      }
                    />
                  </div>
                  ...
                  <div className="flex-1">
                    <p
                      className={`text-[12px] font-semibold ${
                        lmsLocked ? "text-[#9AA5BE]" : "text-emerald-700"
                      }`}
                    >
                      Go to LMS
                    </p>
                  ...
                  {lmsLocked && <Lock size={13} className="text-[#9AA5BE]" />}
                </button>
              </nav>
            </div>
          </aside>
