  ...
  return (
    ...
    >
      ...
      <main className="flex-1 px-5 py-12 flex items-center justify-center">
        <div className="w-full max-w-xl bg-white border border-[rgba(27,58,107,0.08)] rounded-3xl shadow-sm overflow-hidden">
          {!data && !error ? (
            <div className="py-20 text-center text-[13px] text-[#5A6A8A]">
            ...
            data && (
              <>
                <div
                  className={`p-6 text-center ${data.is_valid ? "bg-emerald-50" : "bg-red-50"}`}
                >
                  <span
                    className={`w-14 h-14 mx-auto rounded-2xl flex items-center justify-center ${data.is_valid ? "bg-emerald-600 text-white" : "bg-red-600 text-white"}`}
                  >
                    {data.is_valid ? (
                  ...
                  <h1
                    className={`text-[21px] font-semibold mt-4 ${data.is_valid ? "text-emerald-800" : "text-red-800"}`}
                  >
                    {data.is_valid
                      ? "Valid Certificate"
                      : "Certificate Invalid"}
                  ...
                  <p className="text-[11px] mt-1 opacity-70">
                    Verified directly by EduConnect
                  </p>
                </div>
                <div className="p-7 space-y-4">
                  <div>
                    <p className="text-[10px] uppercase tracking-wider text-[#9AA5BE]">
                      Awarded to
                    </p>
                    <p className="text-[18px] font-semibold text-[#0F1C3F] mt-1">
                      {data.student_name}
                    </p>
                  </div>
                  <div>
                    ...
                    <p className="text-[15px] font-medium text-[#0F1C3F] mt-1">
                      {data.course_title}
                    </p>
                  </div>
                  <div className="grid grid-cols-2 gap-4 pt-4 border-t">
                    <div>
                      ...
                      <p className="text-[11.5px] font-semibold mt-1">
                        {data.certificate_number}
                      </p>
                    </div>
                    <div>
                      <p className="text-[10px] text-[#9AA5BE]">Issued on</p>
                      <p className="text-[11.5px] font-semibold mt-1">
                  ...
                  {!data.is_valid && (
                    <div className="p-3 bg-red-50 text-red-700 rounded-xl text-[11.5px]">
