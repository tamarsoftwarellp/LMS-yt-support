...
function VideoPlayer({
  ...
  onProgress?: (
    ...
    duration: number,
  ) => Promise<{ status: string; watched_percentage: number }>;
  ...
  const onAutoCompletedRef = useRef(onAutoCompleted);
  const currentRef = useRef(lesson.resumePosition || 0);
  const durationRef = useRef(totalSecsFallback);
  const lastReportedRef = useRef(lesson.resumePosition || 0);
  const completionReportedRef = useRef(lesson.completed);
  onEndedRef.current = onEnded;
  onProgressRef.current = onProgress;
  onAutoCompletedRef.current = onAutoCompleted;

  const reportProgress = async (position?: number) => {
    const callback = onProgressRef.current;
    const duration = Math.round(durationRef.current);
    const currentPosition = Math.max(
      0,
      Math.round(position ?? currentRef.current),
    );
    const previousPosition = Math.max(0, Math.round(lastReportedRef.current));
    if (
      !callback ||
      !duration ||
      Math.abs(currentPosition - previousPosition) < 1
    )
      return undefined;
    lastReportedRef.current = currentPosition;
    setSaveState("saving");
    try {
      ...
      setWatchedPercentage(result.watched_percentage);
      setSaveState("saved");
      if (result.status === "completed" && !completionReportedRef.current) {
        completionReportedRef.current = true;
        onAutoCompletedRef.current?.();
      }
      return result;
    } catch {
      lastReportedRef.current = previousPosition;
      setSaveState("error");
      return undefined;
    }
  };
    ...
    loadYouTubeApi().then(() => {
      ...
      if (!playerRef.current) {
        playerRef.current = new window.YT.Player(hostRef.current, {
          ...
          events: {
            ...
            onStateChange: (e: any) => {
              ...
              if (e.data === YT.PAUSED) {
                setPlaying(false);
                void reportProgress(e.target.getCurrentTime?.());
              }
              if (e.data === YT.ENDED) {
                setPlaying(false);
                void reportProgress(e.target.getDuration?.()).then((result) => {
                  if (
        ...
        playerRef.current.loadVideoById({
          videoId: lesson.youtubeId,
          startSeconds: lesson.resumePosition || 0,
        });
        setCurrent(lesson.resumePosition || 0);
        currentRef.current = lesson.resumePosition || 0;
        lastReportedRef.current = lesson.resumePosition || 0;
        setWatchedPercentage(lesson.watchedPercentage || 0);
        completionReportedRef.current = lesson.completed;
        setPlaying(false);
      }
    });
  ...
  useEffect(() => {
    return () => {
      void reportProgress(playerRef.current?.getCurrentTime?.());
  ...
  useEffect(() => {
    if (playing && ready) {
      pollRef.current = setInterval(() => {
        ...
        if (!p) return;
        const next = p.getCurrentTime?.() ?? 0;
        const duration = p.getDuration?.() || totalSecsFallback;
        setCurrent(next);
  ...
  const changeQuality = (value: string) => {
    setQuality(value);
    if (value !== "auto") playerRef.current?.setPlaybackQuality?.(value);
  ...
  return (
    <div
      ref={containerRef}
      className="relative bg-black rounded-xl overflow-hidden group fullscreen:rounded-none fullscreen:w-screen fullscreen:h-screen fullscreen:flex fullscreen:items-center fullscreen:justify-center"
    >
      {/* video canvas */}
      <div
      ...
      >
        {lesson.resumePosition &&
        !playing &&
        current === lesson.resumePosition ? (
          <div className="absolute top-3 right-3 z-10 px-2.5 py-1 rounded-lg bg-black/70 text-white/80 text-[11px]">
            Resumed at {fmt(lesson.resumePosition)}
          </div>
        ) : null}
        <div
          className={`absolute top-3 left-3 z-10 px-2.5 py-1 rounded-lg text-[10.5px] ${saveState === "error" ? "bg-red-600 text-white" : "bg-black/70 text-white/70"}`}
        >
          {saveState === "saving"
            ? "Saving…"
            : saveState === "error"
        ...
        {/* title overlay */}
        <p className="absolute bottom-14 left-4 text-white/60 text-[13px] font-medium drop-shadow pointer-events-none">
          {lesson.title}
        </p>
      </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/90 to-transparent px-4 pb-3 pt-8 opacity-0 group-hover:opacity-100 transition-opacity">
        ...
        >
          ...
          />
          <div
            className="absolute top-1/2 -translate-y-1/2 w-3.5 h-3.5 bg-white rounded-full shadow-lg -ml-1.5 transition-none"
            style={{ left: `${pct}%` }}
          />
        </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            ...
            {showVolume && (
              <div className="absolute bottom-8 left-1/2 -translate-x-1/2 bg-black/90 rounded-xl p-3 shadow-xl border border-white/10">
                ...
                <button
                  onClick={toggleMute}
                  className="mt-2 text-white/60 hover:text-white text-[10px] block w-full text-center"
                >
                  {muted ? "Unmute" : "Mute"}
                </button>
              </div>
          ...
          <div className="relative">
            <button
              className="text-white/70 hover:text-white text-[12px] font-bold transition-colors px-1"
          ...
          <div className="relative">
            <button
              ...
              className={`text-white/70 hover:text-white transition-colors ${showSettings ? "text-white" : ""}`}
              onClick={() => {
                setQualityLevels(
            ...
            {showSettings && (
              <div className="absolute bottom-8 right-0 w-52 bg-black/95 rounded-xl p-2 shadow-xl border border-white/10 text-[11px] text-white">
                <p className="px-2 py-1 text-white/45 uppercase tracking-wider">
                  Quality
                </p>
                <button
                  onClick={() => changeQuality("auto")}
                  className={`w-full px-2 py-2 rounded-lg flex justify-between hover:bg-white/10 ${quality === "auto" ? "text-white font-semibold" : "text-white/70"}`}
                >
                  <span>Auto</span>
                  {quality === "auto" && <Check size={12} />}
                </button>
                {qualityLevels.map((level) => (
                  <button
                    key={level}
                    onClick={() => changeQuality(level)}
                    className={`w-full px-2 py-2 rounded-lg flex justify-between hover:bg-white/10 ${quality === level ? "text-white font-semibold" : "text-white/70"}`}
                  >
                    <span>
                      {level
                        .replace("hd", "HD ")
                    ...
                    {quality === level && <Check size={12} />}
                  </button>
                ))}
                <div className="my-1 border-t border-white/10" />
                <button
                  onClick={toggleCaptions}
                  className="w-full px-2 py-2 rounded-lg flex justify-between hover:bg-white/10 text-white/70"
                >
                  <span>Captions</span>
                  <span
                    className={captions ? "text-emerald-400" : "text-white/40"}
                  >
                    {captions ? "On" : "Off"}
                  </span>
                </button>
...
function NotesTab({ lessonTitle }: { lessonTitle: string }) {
  const [notes, setNotes] = useState([
    {
      id: "n1",
      time: "2:15",
      text: "Key concept: hooks allow functional components to manage state.",
    },
    {
      id: "n2",
      time: "8:40",
      text: "Remember to always call hooks at the top level, never inside loops.",
    },
  ]);
  ...
  const add = () => {
    ...
    setNotes((n) => [
      ...n,
      { id: Date.now().toString(), time: "0:00", text: draft.trim() },
    ]);
    setDraft("");
  };
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2">
        ...
        <button
          onClick={add}
          className="self-end px-4 py-2 bg-[#1B3A6B] text-white text-[12px] font-semibold rounded-xl hover:bg-[#152d54] transition-colors"
        >
          Save
        </button>
      </div>
      {notes.map((n) => (
        <div
          key={n.id}
          className="flex gap-3 p-3 bg-white rounded-xl border border-[rgba(27,58,107,0.1)] group"
        >
          <span className="shrink-0 text-[11px] font-bold text-[#1B3A6B] bg-[#EBF1FA] px-2 py-0.5 rounded-md mt-0.5 h-fit">
            {n.time}
          </span>
          <p className="text-[13px] text-[#374151] flex-1">{n.text}</p>
          <button
            onClick={() => setNotes((ns) => ns.filter((x) => x.id !== n.id))}
            className="opacity-0 group-hover:opacity-100 text-[#9AA5BE] hover:text-red-400 transition-all"
          >
            <X size={14} />
          </button>
        </div>
  ...
  const [questions, setQuestions] = useState([
    {
      ...
      answered: true,
      answer:
        "useCallback memoizes a function reference; useMemo memoizes the result of calling a function. Use useCallback when passing callbacks to child components to prevent re-renders, and useMemo for expensive computations.",
    },
    {
      id: "q2",
      user: "Rahul M.",
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-2">
        ...
        <button
          onClick={ask}
          className="px-4 py-2.5 bg-[#1B3A6B] text-white rounded-xl hover:bg-[#152d54] transition-colors"
        >
          <Send size={14} />
        </button>
      </div>
      ...
      {questions.map((item) => (
        <div
          key={item.id}
          className="bg-white rounded-xl border border-[rgba(27,58,107,0.1)] p-4 space-y-3"
        >
          <div className="flex gap-2.5">
            <div className="w-7 h-7 rounded-full bg-[#1B3A6B] flex items-center justify-center text-white text-[10px] font-bold shrink-0">
              {item.avatar}
            </div>
            <div className="flex-1">
              <div className="flex items-center gap-2 mb-1">
                <span className="text-[12.5px] font-semibold text-[#0F1C3F]">
                  {item.user}
                </span>
          ...
          {item.answered && (
            <div className="ml-9 p-3 bg-[#F4F7FC] rounded-lg border-l-2 border-[#1B3A6B]">
              <p className="text-[11.5px] font-semibold text-[#1B3A6B] mb-1">
                Instructor reply
              </p>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-center gap-3 p-4 bg-white rounded-xl border border-[rgba(27,58,107,0.1)]">
        ...
        <div>
          ...
          <div className="flex items-center gap-3 mt-1">
            ...
            <span className="flex items-center gap-1 text-[11.5px] text-[#5A6A8A]">
              <Users size={11} />
      ...
      <div className="bg-white rounded-xl border border-[rgba(27,58,107,0.1)] p-4">
        <p className="text-[13.5px] font-bold text-[#0F1C3F] mb-2">
          About this course
        </p>
      ...
      {/* what you'll learn */}
      <div className="bg-white rounded-xl border border-[rgba(27,58,107,0.1)] p-4">
        <p className="text-[13.5px] font-bold text-[#0F1C3F] mb-3">
          What you will learn
        </p>
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
          {course.skills.map((s) => (
            <div
              key={s}
              className="flex items-start gap-2 text-[12.5px] text-[#374151]"
            >
...
function ArticleView({ lesson }: { lesson: Lesson }) {
  return (
    <div className="bg-white rounded-xl border border-[rgba(27,58,107,0.1)] p-4 sm:p-6">
      {lesson.articleContent?.trim() ? (
        <div className="text-[13.5px] text-[#374151] leading-7 whitespace-pre-wrap break-words">
          {lesson.articleContent}
        </div>
      ) : (
        <div className="py-8 text-center text-[13px] text-[#5A6A8A]">
...
function QuizView() {
  return (
    <div className="p-6 bg-white border border-slate-200 rounded-xl text-[13px] text-[#5A6A8A]">
...
function AssignmentView() {
  return (
    <div className="p-6 bg-white border border-slate-200 rounded-xl text-[13px] text-[#5A6A8A]">
      This assignment is not configured yet.
    </div>
  );
}
...
export function CoursePlayer({
  ...
  course: CourseData;
  onBack: () => void;
  onLessonComplete?: (lessonId: string) => Promise<void> | void;
  onVideoProgress?: (
    lessonId: string,
    previousPosition: number,
    currentPosition: number,
  ...
  renderQuiz?: (lesson: Lesson, onPassed: () => void) => ReactNode;
  renderAssignment?: (lesson: Lesson, onPassed: () => void) => ReactNode;
}) {
  ...
  const markDone = (advance = true) => {
    if (!completedIds.includes(activeLesson.id)) {
      ...
      if (activeLesson.type !== "quiz" && activeLesson.type !== "assignment")
        void onLessonComplete?.(activeLesson.id);
    }
    if (advance && curIdx < allUnlocked.length - 1)
      setActiveLesson(allUnlocked[curIdx + 1]);
  };
  ...
  return (
    <div
      className="relative flex h-screen overflow-hidden bg-[#1a1a1a]"
    ...
    >
      ...
      <aside
        className={`${sidebarOpen ? "w-[300px] translate-x-0" : "w-[300px] -translate-x-full md:w-0"} absolute md:relative inset-y-0 left-0 z-30 bg-[#1a1a1a] border-r border-white/10 flex flex-col shrink-0 overflow-hidden transition-all duration-200`}
      >
        <div className="p-4 border-b border-white/10">
          <button
            onClick={onBack}
            className="flex items-center gap-1.5 text-[12px] text-white/50 hover:text-white/80 transition-colors mb-3"
          >
            <ArrowLeft size={13} /> Back to courses
          </button>
          <p className="text-[13px] font-bold text-white line-clamp-2 leading-snug">
            {course.title}
          </p>
        ...
        <div className="flex-1 overflow-y-auto py-2">
          {course.sections.map((sec, si) => {
            const secDone = sec.lessons.filter((l) =>
            ...
            return (
              <div key={sec.id}>
                <button
                  onClick={() =>
                    setExpanded((ex) =>
                      isOpen ? ex.filter((e) => e !== sec.id) : [...ex, sec.id],
                    )
                  }
                ...
                >
                  <ChevronDown
                    size={13}
                    className={`text-white/40 transition-transform shrink-0 ${isOpen ? "" : "-rotate-90"}`}
                  />
                  <div className="flex-1 min-w-0">
                    <p className="text-[12px] font-semibold text-white/80 leading-snug">
                      <span className="text-white/30 mr-1">S{si + 1}.</span>
                      {sec.title}
                    </p>
                ...
                {isOpen &&
                  sec.lessons.map((lesson, li) => {
                    ...
                    return (
                      ...
                      >
                        <div
                          className={`w-4.5 h-4.5 rounded-full flex items-center justify-center shrink-0 mt-0.5 border
                        ...
                        <div className="flex-1 min-w-0">
                          <p
                            className={`text-[11.5px] leading-snug ${isActive ? "text-white font-semibold" : isDone ? "text-white/60" : "text-white/50"}`}
                          >
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            ...
                            {lesson.isPreview && (
                              <span className="text-[9.5px] text-amber-400 font-medium">
                                Preview
                              </span>
                            )}
                          </div>
        ...
        <div className="bg-[#0A1629] px-4 py-2.5 flex items-center gap-3 shrink-0">
          ...
          <div className="flex items-center gap-2 shrink-0">
            <button
              disabled={curIdx === 0}
              onClick={() =>
                curIdx > 0 && setActiveLesson(allUnlocked[curIdx - 1])
              }
            ...
            <button
              disabled={curIdx >= allUnlocked.length - 1}
              onClick={() =>
                curIdx < allUnlocked.length - 1 &&
                setActiveLesson(allUnlocked[curIdx + 1])
              }
              className="flex items-center gap-1 px-2.5 py-1.5 text-[11.5px] text-white/60 hover:text-white border border-white/10 hover:border-white/30 rounded-lg disabled:opacity-30 transition-colors"
            >
        ...
        <div className="flex-1 overflow-y-auto">
          <div className="max-w-[820px] mx-auto px-3 sm:px-6 py-4 sm:py-6">
            ...
            <div className="flex items-center gap-2 mb-2">
              ...
              >
                <Icon size={10} />
                {activeLesson.type.charAt(0).toUpperCase() +
                  activeLesson.type.slice(1)}
              </span>
              {activeLesson.type === "video" && (
                <span className="text-[12px] text-[#9AA5BE] flex items-center gap-1">
                  <Clock size={11} />
                  {activeLesson.duration}m
                </span>
              )}
            </div>
            <h2
              className="text-[22px] font-bold text-[#0F1C3F] mb-4 leading-tight"
            ...
            {activeLesson.type === "video" && (
              <VideoPlayer
                lesson={activeLesson}
                color={course.color}
                onEnded={() => markDone(true)}
                onAutoCompleted={() => markDone(false)}
                onProgress={(previous, current, duration) =>
                  onVideoProgress?.(
                    activeLesson.id,
                    previous,
                    current,
            ...
            {activeLesson.type === "article" && (
              <ArticleView lesson={activeLesson} />
            )}
            {activeLesson.type === "quiz" &&
              (renderQuiz ? (
                renderQuiz(activeLesson, () => markDone(true))
              ) : (
                <QuizView />
              ))}
            {activeLesson.type === "assignment" &&
              (renderAssignment ? (
                renderAssignment(activeLesson, () => markDone(true))
              ) : (
                <AssignmentView />
              ))}
            ...
            <div className="mt-5">
              ...
              {activeTab === "notes" && (
                <NotesTab lessonTitle={activeLesson.title} />
              )}
              {activeTab === "qa" && <QATab />}
            </div>
          </div>
