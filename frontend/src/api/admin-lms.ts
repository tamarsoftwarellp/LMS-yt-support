...
export type AdminAssignmentInput = Omit<
  AdminAssignment,
  "id" | "lesson_id" | "status"
>;
...
export function hasAdminSession() {
  return Boolean(
    sessionStorage.getItem(ACCESS_KEY) || localStorage.getItem(REFRESH_KEY),
  );
}
  ...
  if (!response.ok) {
    ...
    const message =
      typeof body?.detail === "string"
        ? body.detail
        : validationDetail || "Request failed. Please try again.";
...
export async function adminDownload(path: string, fileName: string) {
  ...
  if (response.status === 401) {
    access = await refreshAdminAccessToken();
...
export const getCurrentAdmin = () =>
  adminApiRequest<AdminProfile>("/api/v1/auth/admin/me");
...
export const loadCourses = (params: LoadCoursesParams = {}) => {
  ...
  return adminApiRequest<PaginatedCourses>(
    `/api/v1/admin/courses${query.toString() ? `?${query.toString()}` : ""}`,
  );
};

export const loadCourse = (courseId: string) =>
  adminApiRequest<AdminCourseDetail>(`/api/v1/admin/courses/${courseId}`);
export const loadPublicationReadiness = (courseId: string) =>
  adminApiRequest<PublicationReadiness>(
    `/api/v1/admin/courses/${courseId}/publication-readiness`,
  );
export const createCourse = (payload: CourseInput) =>
  adminApiRequest<AdminCourseDetail>("/api/v1/admin/courses", {
    method: "POST",
    body: JSON.stringify(payload),
  });
export const updateCourse = (courseId: string, payload: CourseUpdateInput) =>
  adminApiRequest<AdminCourseDetail>(`/api/v1/admin/courses/${courseId}`, {
    method: "PUT",
    body: JSON.stringify(payload),
  });
export const deleteCourse = (courseId: string) =>
  adminApiRequest<{ detail: string }>(`/api/v1/admin/courses/${courseId}`, {
    method: "DELETE",
  });
export const publishCourse = (courseId: string) =>
  adminApiRequest<AdminCourseDetail>(
...
export const restoreCourse = (courseId: string) =>
  adminApiRequest<AdminCourseDetail>(
    `/api/v1/admin/courses/${courseId}/restore`,
    { method: "POST" },
  );
export const createSection = (courseId: string, payload: SectionInput) =>
  adminApiRequest<AdminSection>(`/api/v1/admin/courses/${courseId}/sections`, {
    method: "POST",
    body: JSON.stringify(payload),
  });
export const updateSection = (sectionId: string, payload: SectionInput) =>
  adminApiRequest<AdminSection>(`/api/v1/admin/sections/${sectionId}`, {
    method: "PUT",
    body: JSON.stringify(payload),
  });
export const deleteSection = (sectionId: string) =>
  adminApiRequest<{ detail: string }>(`/api/v1/admin/sections/${sectionId}`, {
    method: "DELETE",
  });
export const reorderSections = (courseId: string, payload: ReorderInput) =>
  adminApiRequest<AdminCourseDetail>(
    `/api/v1/admin/courses/${courseId}/sections/reorder`,
    { method: "PUT", body: JSON.stringify(payload) },
  );
export const createLesson = (sectionId: string, payload: LessonInput) =>
  adminApiRequest<AdminLesson>(`/api/v1/admin/sections/${sectionId}/lessons`, {
    method: "POST",
    body: JSON.stringify(payload),
  });
export const updateLesson = (lessonId: string, payload: LessonInput) =>
  adminApiRequest<AdminLesson>(`/api/v1/admin/lessons/${lessonId}`, {
    method: "PUT",
    body: JSON.stringify(payload),
  });
export const deleteLesson = (lessonId: string) =>
  adminApiRequest<{ detail: string }>(`/api/v1/admin/lessons/${lessonId}`, {
    method: "DELETE",
  });
export const reorderLessons = (sectionId: string, payload: ReorderInput) =>
  adminApiRequest<AdminSection>(
    `/api/v1/admin/sections/${sectionId}/lessons/reorder`,
    { method: "PUT", body: JSON.stringify(payload) },
  );
export const loadAdminQuiz = (lessonId: string) =>
  adminApiRequest<AdminQuiz | null>(`/api/v1/admin/lessons/${lessonId}/quiz`);
export const saveAdminQuiz = (lessonId: string, payload: AdminQuizInput) =>
  adminApiRequest<AdminQuiz>(`/api/v1/admin/lessons/${lessonId}/quiz`, {
    method: "PUT",
    body: JSON.stringify(payload),
  });
export const publishAdminQuiz = (quizId: string) =>
...
export const saveAdminAssignment = (
  lessonId: string,
  payload: AdminAssignmentInput,
) =>
  adminApiRequest<AdminAssignment>(
    `/api/v1/admin/lessons/${lessonId}/assignment`,
    { method: "PUT", body: JSON.stringify(payload) },
  );
export const publishAdminAssignment = (assignmentId: string) =>
  adminApiRequest<AdminAssignment>(
    `/api/v1/admin/assignments/${assignmentId}/publish`,
    { method: "POST" },
  );
export const loadAssignmentSubmissions = () =>
  adminApiRequest<AdminAssignmentSubmission[]>(
    "/api/v1/admin/assignment-submissions",
  );
export const evaluateAssignmentSubmission = (
  ...
  adminApiRequest<AdminAssignmentSubmission>(
    `/api/v1/admin/assignment-submissions/${submissionId}/evaluate`,
    { method: "POST", body: JSON.stringify(payload) },
  );
export const loadAdminAnalytics = (days = 30) =>
  adminApiRequest<AdminAnalyticsOverview>(
    `/api/v1/admin/analytics/overview?days=${days}`,
  );
export const loadAdminCertificates = (search = "", status = "") => {
  const query = new URLSearchParams();
  if (search.trim()) query.set("search", search.trim());
  if (status) query.set("status", status);
  return adminApiRequest<AdminCertificate[]>(
    `/api/v1/admin/certificates${query.toString() ? `?${query}` : ""}`,
  );
};
export const revokeCertificate = (id: string, reason: string) =>
  adminApiRequest<AdminCertificate>(`/api/v1/admin/certificates/${id}/revoke`, {
    method: "POST",
    body: JSON.stringify({ reason }),
  });
export const reissueCertificate = (id: string) =>
  adminApiRequest<AdminCertificate>(
    `/api/v1/admin/certificates/${id}/reissue`,
    { method: "POST" },
  );
export async function downloadAssignmentSubmissionFile(
  submissionId: string,
  fileName: string,
) {
  let access = sessionStorage.getItem(ACCESS_KEY);
  ...
  let response = await fetch(
    `${API_URL}/api/v1/admin/assignment-submissions/${submissionId}/file`,
    { headers: { Authorization: `Bearer ${access}` } },
  );
  if (response.status === 401) {
    access = await refreshAdminAccessToken();
  ...
  if (!response.ok) throw new Error("Unable to download submission file.");
  const url = URL.createObjectURL(await response.blob());
  const anchor = document.createElement("a");
  anchor.href = url;
  anchor.download = fileName;
