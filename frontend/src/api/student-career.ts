...
export interface StudentCoding {id:string;lesson_id:string;instructions:string;function_name:string;starter_code:string;maximum_attempts:number;attempts_used:number;remaining_attempts:number;passed:boolean;last_code:string;test_cases:{input:unknown[];expected:unknown}[]}
export interface CodingSubmitResult {submission_id:string;attempt_number:number;passed_count:number;total_count:number;passed:boolean;certificate_issued?:{id:string;certificate_number:string;verification_token:string}|null}
...
export const loadStudentCoding = (lessonId:string) => studentApiRequest<StudentCoding>(`/api/v1/students/me/lessons/${lessonId}/coding`);
export const submitCodingChallenge = (challengeId:string,input:{code:string;passed_count:number;total_count:number}) => studentApiRequest<CodingSubmitResult>(`/api/v1/students/me/coding-challenges/${challengeId}/submit`,{method:"POST",body:JSON.stringify(input)});
